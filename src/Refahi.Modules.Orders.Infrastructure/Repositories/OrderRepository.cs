...
public class OrderRepository : IOrderRepository
{
    ...
    public async Task<Order?> GetByIdempotencyKeyWithItemsAsync(
        string idempotencyKey,
        CancellationToken ct = default
    )
    {
    ...
    public async Task<List<Order>> GetBySourceAsync(
        string sourceModule,
    ...
    )
    {
        return await _context
            .Orders.Where(o =>
                o.SourceModule == sourceModule && o.SourceReferenceId == sourceReferenceId
            )
            .OrderByDescending(o => o.CreatedAt)
            .Skip((page - 1) * pageSize)
            .Take(pageSize)
    ...
    public async Task<int> CountBySourceAsync(
        string sourceModule,
        Guid sourceReferenceId,
        CancellationToken ct = default
    )
    {
        return await _context.Orders.CountAsync(
            o => o.SourceModule == sourceModule && o.SourceReferenceId == sourceReferenceId,
            ct
        );
    }
    ...
    )
    {
        var query = _context
            ...
            .Where(x =>
                ...
                && (
                    ...
                    || (
                        x.SourceShopId.HasValue
                        && ownShopIds.Contains(x.SourceShopId.Value)
        ...
        if (shopId.HasValue)
            query = query.Where(x => x.SourceShopId == shopId.Value);
        if (from.HasValue)
    ...
    )
    {
        ...
        return (
            await query.CountAsync(
                x => x.Status == OrderStatus.Pending || x.Status == OrderStatus.Confirmed,
                ct
            ),
