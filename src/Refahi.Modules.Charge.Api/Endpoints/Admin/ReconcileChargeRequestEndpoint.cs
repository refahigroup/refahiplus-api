...
public sealed class ReconcileChargeRequestEndpoint : IEndpoint
{
    public void Map(object app)
    {
        ...
        routes
            .MapPost(
                "admin/charge-requests/{id:guid}/reconcile",
                async (Guid id, [FromQuery] bool force, ISender sender, CancellationToken ct) =>
                {
                    var result = await sender.Send(
                        new ReconcileChargeRequestCommand(id, force),
                        ct
                    );
