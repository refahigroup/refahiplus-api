...
public sealed class SecureChargeRequestEndpoints : IEndpoint
{
    public void Map(object app)
    {
        if (app is not IEndpointRouteBuilder r) return;
        r.MapGet("charge-requests/{requestId:guid}/pins", async (Guid requestId, HttpContext http, ISender sender, CancellationToken ct) =>
        {
            ...
            if (result is null) return Results.NotFound(ApiResponseHelper.Error("درخواست شارژ یافت نشد", statusCode: 404));
            http.Response.Headers.CacheControl = "no-store";
            http.Response.Headers.Pragma = "no-cache";
            return Results.Ok(ApiResponseHelper.Success(result));
        }).RequireAuthorization("UserOrAdmin").WithName("Charge.Requests.GetPins").WithTags("Charge.Requests")
            .Produces<ApiResponse<IReadOnlyList<ChargePinDeliveryDto>>>(StatusCodes.Status200OK)
            .Produces<ApiErrorResponse>(StatusCodes.Status400BadRequest)
            .Produces<ApiErrorResponse>(StatusCodes.Status404NotFound)
            .Produces(StatusCodes.Status401Unauthorized);

        r.MapPost("charge-requests/{requestId:guid}/cancel", async (Guid requestId, HttpContext http, ISender sender, CancellationToken ct) =>
        {
            if (!ChargeEndpointHelpers.TryUserId(http, out var userId)) return Results.Unauthorized();
        ...
        }).RequireAuthorization("UserOrAdmin").WithName("Charge.Requests.Cancel").WithTags("Charge.Requests")
