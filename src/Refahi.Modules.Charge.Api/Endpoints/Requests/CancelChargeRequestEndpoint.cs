...
public sealed class CancelChargeRequestEndpoint : IEndpoint
{
    public void Map(object app)
    {
        ...
        routes
            .MapPost(
                "charge-requests/{requestId:guid}/cancel",
                async (Guid requestId, HttpContext http, ISender sender, CancellationToken ct) =>
                {
                    ...
                    var cancelled = await sender.Send(
                        new CancelChargeRequestCommand(requestId, userId),
                        ct
                    );
                    return cancelled
                        ? Results.Ok(
                            ApiResponseHelper.Success(
                                new ChargeOperationResponse(requestId),
                                "درخواست لغو شد"
                            )
                        )
                        : Results.NotFound(
                            ApiResponseHelper.Error("درخواست شارژ یافت نشد", statusCode: 404)
                        );
                }
            )
