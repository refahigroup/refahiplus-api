...
public sealed class ConvertHotelRequestToOrderEndpoint : IEndpoint
{
    public void Map(object app)
    {
        ...
        routes
            .MapPost(
                "hotel-requests/{requestId:guid}/order",
                async (
                    Guid requestId,
                ...
                ) =>
                {
                    ...
                    var result = await sender.Send(
                        new ConvertHotelRequestToOrderCommand(requestId, userId, idempotencyKey),
                        ct
                    );

                    return Results.Ok(
                        ApiResponseHelper.Success(result, "سفارش هتل آماده پرداخت شد")
                    );
                }
            )
