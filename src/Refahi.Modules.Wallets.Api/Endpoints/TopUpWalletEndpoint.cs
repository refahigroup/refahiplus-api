                ...
                ) =>
                {
                    if (!httpRequest.Headers.TryGetValue("Idempotency-Key", out var idemKeyValues))
                        return Results.BadRequest(
                            new ProblemDetails { Title = "Missing Idempotency-Key header" }
                        );
                        ...
                        if (resp.Status == CommandStatus.InProgress)
                            return Results.Conflict(
                                new ProblemDetails
                                {
                                    Title = "IN_PROGRESS",
                                    Detail =
                                        "The operation is currently pending. Retry later using the same Idempotency-Key.",
                                }
                            );
                    ...
                    {
                        return Results.BadRequest(
                            new ProblemDetails { Title = cm.Code, Detail = cm.Message }
                        );
                    }
                    catch (IdempotencyKeyConflictException ic)
                    {
                        return Results.Conflict(
                            new ProblemDetails { Title = ic.Code, Detail = ic.Message }
                        );
                    }
                    catch (WalletOperationNotAllowedException wo)
                    {
                        return Results.Conflict(
                            new ProblemDetails { Title = wo.Code, Detail = wo.Message }
                        );
                    }
                }
            )
