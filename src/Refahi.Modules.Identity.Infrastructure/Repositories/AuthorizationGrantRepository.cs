...
public sealed class AuthorizationGrantRepository(IdentityDbContext db) : IAuthorizationGrantRepository
{
    ...
    public Task<AuthorizationGrant?> GetAsync(Guid userId, string issuer, string value, CancellationToken ct = default) =>
        db.AuthorizationGrants.SingleOrDefaultAsync(
            x => x.UserId == userId && x.Issuer == issuer && x.Value == value, ct);

    public async Task<IReadOnlyList<AuthorizationGrant>> GetActiveAsync(
        Guid userId, string? issuer = null, CancellationToken ct = default) =>
        await db.AuthorizationGrants.AsNoTracking()
            .Where(x => x.UserId == userId && x.IsActive && (issuer == null || x.Issuer == issuer))
            .OrderBy(x => x.CreatedAt)
            .ToListAsync(ct);

    public async Task<IReadOnlyList<AuthorizationGrant>> GetByIssuerAsync(string issuer, CancellationToken ct = default) =>
        await db.AuthorizationGrants.AsNoTracking()
