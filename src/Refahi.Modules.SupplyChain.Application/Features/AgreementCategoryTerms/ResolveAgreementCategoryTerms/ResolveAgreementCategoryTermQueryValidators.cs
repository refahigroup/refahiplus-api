...
public sealed class ResolveAgreementCategoryTermQueryValidator : AbstractValidator<ResolveAgreementCategoryTermQuery>
{
    ...
    : AbstractValidator<ResolveAgreementCategoryTermsBatchQuery>
{
    public ResolveAgreementCategoryTermsBatchQueryValidator()
    {
        RuleFor(x => x.Requests)
            .NotNull().WithMessage("درخواست‌های حل قرارداد الزامی است")
            .Must(x => x.Count <= 1000).WithMessage("حداکثر ۱۰۰۰ درخواست در هر batch مجاز است");

        RuleForEach(x => x.Requests).ChildRules(item =>
        {
            item.RuleFor(x => x.SupplierId).NotEmpty().WithMessage("شناسه تامین‌کننده الزامی است");
            item.RuleFor(x => x.CategoryId).GreaterThan(0).WithMessage("شناسه دسته‌بندی الزامی است");
