...
public class CreateAgreementCommandHandler
    : IRequestHandler<CreateAgreementCommand, CreateAgreementResponse>
{
    ...
    public async Task<CreateAgreementResponse> Handle(
    ...
    )
    {
        ...
        var supplierExists = await _supplierRepository.GetByIdAsync(
            request.SupplierId,
            false,
            cancellationToken
        );
