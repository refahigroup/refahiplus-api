...
public interface IShopProductRepository
{
    ...
    Task<ShopProduct?> GetWithVariantOfferingsAsync(
        Guid shopId,
        Guid productId,
        CancellationToken ct = default
    );
    ...
    [Obsolete(
        "Use GetDisplayableShopIdsByAgreementProductIdsAsync which also enforces IsAvailable/IsDeleted on Product."
    )]
    ...
    Task AddVariantOfferingsAsync(
        ShopProduct shopProduct,
        IReadOnlyList<ShopProductVariant> offerings,
        CancellationToken ct = default
    );
    Task UpsertVariantOfferingAsync(
        ShopProduct shopProduct,
