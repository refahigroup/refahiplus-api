...
public sealed class Product
{
    ...
    )
    {
        ...
        if (string.IsNullOrWhiteSpace(title) || string.IsNullOrWhiteSpace(slug))
            throw new StoreDomainException(
                "عنوان و اسلاگ محصول الزامی است",
                "INVALID_PRODUCT_CONTENT"
            );
    ...
    /// </summary>
    public VariantAttributeValue AddVariantAttributeValue(
        Guid attributeId,
        string value,
        int sortOrder = 0
    )
    {
        var attr =
            _variantAttributes.FirstOrDefault(a => a.Id == attributeId)
            ?? throw new StoreDomainException(
                "اتریبیوت تنوع یافت نشد",
    ...
    public void UpdateVariantAttributeValue(
        Guid attributeId,
        Guid valueId,
        string value,
        int sortOrder = 0
    ...
    )
    {
        ...
        {
            var attr =
                _variantAttributes.FirstOrDefault(a => a.Id == attrId)
                ?? throw new StoreDomainException(
                    "اتریبیوت تنوع یافت نشد",
                    "VARIANT_ATTRIBUTE_NOT_FOUND"
                );
            if (!attr.Values.Any(v => v.Id == valueId))
                throw new StoreDomainException(
                    "مقدار اتریبیوت به این اتریبیوت تعلق ندارد",
