...
public sealed class Cart
{
    ...
    public void RefreshOfferItem(Guid itemId, Guid offerId, Guid productId, Guid shopId,
        ...
        long originalUnitPriceMinor, long finalUnitPriceMinor)
    {
        ...
        item.RefreshOffer(offerId, productId, shopId, variantId, sessionId, usageDate,
            quantity, originalUnitPriceMinor, finalUnitPriceMinor);
        UpdatedAt = DateTimeOffset.UtcNow;
    }

    public void AddOfferItem(Guid shopId, Guid productId, Guid offerId, Guid? variantId,
        ...
        long finalUnitPriceMinor)
    {
        ...
        var existing = _items.FirstOrDefault(x => x.OfferId == offerId && x.VariantId == variantId &&
            x.SessionId == sessionId && x.UsageDate == usageDate);
        if (existing is not null) existing.UpdateQuantity(existing.Quantity + quantity);
        else _items.Add(CartItem.CreateFromOffer(Id, shopId, productId, offerId, variantId,
            sessionId, usageDate, quantity, originalUnitPriceMinor, finalUnitPriceMinor));
        UpdatedAt = DateTimeOffset.UtcNow;
    }
