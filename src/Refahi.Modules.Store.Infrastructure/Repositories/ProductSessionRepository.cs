...
public class ProductSessionRepository : IProductSessionRepository
{
    ...
    public Task<List<ProductSession>> GetByProductIdAsync(
        ...
        _db
            .ProductSessions.Where(s => s.ProductId == productId)
            .OrderBy(s => s.Date)
    ...
    public Task<List<ProductSession>> GetByProductIdAndDateAsync(
        ...
        _db
            .ProductSessions.Where(s => s.ProductId == productId && s.Date == date)
    ...
    public Task<List<ProductSession>> GetAvailableByProductIdAsync(
        ...
        _db
            .ProductSessions.Where(s =>
                s.ProductId == productId && s.IsActive && !s.IsCancelled && s.SoldCount < s.Capacity
            )
            .OrderBy(s => s.Date)
            .ThenBy(s => s.StartTime)
            .ToListAsync(ct);
