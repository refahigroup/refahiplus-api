        ...
        CancellationToken ct = default)
    {
        var query =
            ...
            where offer.IsActive && !offer.IsDeleted && offer.StartDateUtc <= atUtc &&
                  ...
                  shop.Status == ShopStatus.Active && shop.ShopType == ShopType.Online
        ...
        if (!string.IsNullOrWhiteSpace(search))
        {
            ...
            query = query.Where(x => x.Product.Title.Contains(term) ||
        ...
        return await query
            .OrderByDescending(x => x.Product.CreatedAt).ThenBy(x => x.Product.Id)
            .ThenBy(x => x.Offer.FinalPriceMinor).ThenBy(x => x.Offer.Id)
            .Select(x => new PublicCatalogOfferCandidate(
                ...
                x.Product.Images.Where(image => image.IsMain).OrderBy(image => image.SortOrder)
                    .Select(image => image.ImageUrl).FirstOrDefault() ??
                x.Product.Images.OrderBy(image => image.SortOrder).Select(image => image.ImageUrl).FirstOrDefault(),
                x.Shop.Name, x.Shop.Slug, x.Offer.OriginalPriceMinor, x.Offer.DiscountPercent,
                x.Offer.FinalPriceMinor, x.Offer.ProductVariantId, x.Offer.ProductSessionId,
                x.Offer.StartDateUtc, x.Offer.EndDateUtc, x.Offer.Version, x.Offer.UpdatedAt))
            .ToListAsync(ct);
    }
}
