...
{
    public Task<Voucher?> GetByItemSequenceAsync(Guid itemId, int sequence, CancellationToken ct = default) =>
        db.Vouchers.SingleOrDefaultAsync(x => x.StoreOrderItemId == itemId && x.SequenceNumber == sequence, ct);
    ...
    public async Task<IReadOnlyList<Voucher>> GetByUserAsync(Guid userId, CancellationToken ct = default) =>
        await db.Vouchers.AsNoTracking().Where(x => x.UserId == userId)
            .OrderByDescending(x => x.IssuedAtUtc).ThenBy(x => x.SequenceNumber).ToListAsync(ct);
    ...
    public async Task<IReadOnlyList<Voucher>> GetByStoreOrderAsync(Guid id, CancellationToken ct = default) =>
        await db.Vouchers.Where(x => x.StoreOrderId == id).OrderBy(x => x.StoreOrderItemId)
            .ThenBy(x => x.SequenceNumber).ToListAsync(ct);

    public Task<VoucherRedemption?> GetRedemptionByIdempotencyAsync(Guid userId, string key,
        CancellationToken ct = default) => db.VoucherRedemptions.AsNoTracking()
        ...
        Guid supplierId, Guid? shopId, int page, int pageSize, CancellationToken ct = default)
    {
        var query =
            ...
            where redemption.SupplierId == supplierId && voucher.SupplierId == supplierId &&
                  (!shopId.HasValue || redemption.ShopId == shopId.Value)
            select new VoucherRedemptionHistoryRow(
                voucher.Id, voucher.StoreOrderId, voucher.SequenceNumber,
                voucher.ProductId, voucher.ProductTitle, redemption.SupplierId,
                redemption.ShopId, voucher.RedeemedShopName ?? voucher.ShopName,
                redemption.VendorUserId, redemption.RedeemedAtUtc);

        var total = await query.CountAsync(ct);
        var items = await query.OrderByDescending(x => x.RedeemedAtUtc).ThenBy(x => x.VoucherId)
    ...
    public async Task RedeemAsync(Voucher voucher, VoucherRedemption redemption, CancellationToken ct = default)
    {
        ...
        {
            db.ChangeTracker.Clear();
            throw new StoreDomainException("کلید یکتایی قبلاً استفاده شده است", "VOUCHER_IDEMPOTENCY_CONFLICT");
        }
    }
    ...
    public async Task UpdateRangeAsync(IReadOnlyCollection<Voucher> vouchers, CancellationToken ct = default)
    {
