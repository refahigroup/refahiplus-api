...
{
    public Task<StoreOrder?> GetByIdempotencyKeyAsync(Guid userId, string idempotencyKey, CancellationToken ct = default) =>
        db.StoreOrders.Include(x => x.Items)
            .SingleOrDefaultAsync(x => x.UserId == userId && x.IdempotencyKey == idempotencyKey, ct);
    ...
    public async Task<IReadOnlyList<StoreOrder>> GetByOrderIdsAsync(IReadOnlyCollection<Guid> orderIds,
        CancellationToken ct = default)
    {
        if (orderIds.Count == 0) return [];
        return await db.StoreOrders.AsNoTracking().Include(x => x.Items)
    ...
    public async Task AddAsync(StoreOrder order, CancellationToken ct = default)
    {
        ...
        catch (DbUpdateException ex) when (ex.InnerException?.Message.Contains("IX_store_orders_UserId_IdempotencyKey", StringComparison.OrdinalIgnoreCase) == true)
        { throw new StoreDomainException("کلید یکتایی قبلاً استفاده شده است", "IDEMPOTENCY_CONFLICT"); }
    }
    ...
    public async Task CommitPaidAsync(Guid orderId, CancellationToken ct = default)
    {
        ...
        var storeOrder = await db.StoreOrders.Include(x => x.Items)
        ...
        if (storeOrder.Status != StoreOrderStatus.PendingPayment)
            throw new StoreDomainException("وضعیت سفارش فروشگاه برای پرداخت معتبر نیست", "INVALID_STORE_ORDER_TRANSITION");

        foreach (var item in storeOrder.Items)
        {
            if (item.ProductSessionId.HasValue)
            {
                var session = await db.ProductSessions.SingleOrDefaultAsync(x => x.Id == item.ProductSessionId, ct)
            ...
            else if (item.SalesModel == SalesModel.InventoryBased)
            {
                ...
                if (item.ProductVariantId.HasValue) product.DecreaseVariantStock(item.ProductVariantId.Value, item.Quantity);
                else product.DecreaseStock(item.Quantity);
            }
        }

        var cart = await db.Carts.Include(x => x.Items)
            .SingleOrDefaultAsync(x => x.UserId == storeOrder.UserId && x.ModuleId == storeOrder.ModuleId, ct);
        cart?.Clear();
        storeOrder.MarkPaid();
