...
public sealed class OfferRepository(StoreDbContext db) : IOfferRepository
{
    ...
    public async Task<(IReadOnlyList<Offer> Items, int Total)> GetPagedAsync(
        Guid? productId,
        Guid? shopId,
        bool includeDeleted,
    ...
    )
    {
        ...
        if (productId.HasValue)
            query = query.Where(x => x.offer.ProductId == productId.Value);
        if (shopId.HasValue)
            query = query.Where(x => x.offer.ShopId == shopId.Value);
        return await query
