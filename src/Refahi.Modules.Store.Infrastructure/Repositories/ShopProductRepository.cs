...
public class ShopProductRepository : IShopProductRepository
{
    ...
    )
    {
        try
        {
            ...
            var query =
                ...
                join shopProduct in _db.ShopProducts.AsNoTracking()
                    on offering.ShopProductId equals shopProduct.Id
                join product in _db.Products.AsNoTracking()
                    on shopProduct.ProductId equals product.Id
                join variant in _db.ProductVariants.AsNoTracking()
                    on offering.ProductVariantId equals variant.Id
                join shop in _db.Shops.AsNoTracking() on shopProduct.ShopId equals shop.Id
                where
                    allowedIds.Contains(product.AgreementProductId)
                    && shop.Status == ShopStatus.Active
                    && shopProduct.IsActive
                    && !shopProduct.IsDeleted
                    && offering.IsActive
                    && !offering.IsDeleted
                    && product.IsAvailable
                    && !product.IsDeleted
                    ...
                    && (
                        (
                            stockBasedAgreementProductIds.Contains(product.AgreementProductId)
                        ...
                        || (
                            ...
                            && (
                                (
                                    variant.CapacityType == VariantCapacityType.Unlimited
                                    || variant.Capacity > 0
                                )
            ...
            var labels = await (
                ...
                join attribute in _db.VariantAttributes.AsNoTracking()
                    on combination.VariantAttributeId equals attribute.Id
                join value in _db.VariantAttributeValues.AsNoTracking()
                    on combination.VariantAttributeValueId equals value.Id
                where variantIds.Contains(combination.ProductVariantId)
                orderby attribute.SortOrder, value.SortOrder
                select new
                {
                    combination.ProductVariantId,
                    attribute.Name,
                    value.Value,
                }
            ).ToListAsync(ct);
    ...
    public Task<ShopProduct?> GetAsync(
        ...
        _db.ShopProducts.FirstOrDefaultAsync(
            sp => sp.ShopId == shopId && sp.ProductId == productId && !sp.IsDeleted,
            ct
        );
    ...
    public Task<ShopProduct?> GetWithVariantOfferingsAsync(
        ...
        _db
            .ShopProducts.Include(sp => sp.VariantOfferings)
            .FirstOrDefaultAsync(
                sp => sp.ShopId == shopId && sp.ProductId == productId && !sp.IsDeleted,
                ct
            );
    ...
    {
        var q = _db.ShopProducts.Include(sp => sp.VariantOfferings).Where(sp => !sp.IsDeleted);
        ...
        return await q.OrderBy(sp => sp.ShopId).ThenBy(sp => sp.ProductId).ToListAsync(ct);
    }
        ...
        return await _db
            .ShopProducts.Join(
                _db.Products.Where(p =>
                    idList.Contains(p.AgreementProductId) && p.IsAvailable && !p.IsDeleted
                ),
                sp => sp.ProductId,
                p => p.Id,
                (sp, _) => sp
    ...
    )
    {
        ...
        return await (
            ...
            join p in _db.Products.Where(p =>
                p.IsAvailable && !p.IsDeleted && apIds.Contains(p.AgreementProductId)
            )
    ...
    )
    {
        ...
        var total = await q.CountAsync(ct);
        var productIds = await q.Skip((page - 1) * pageSize).Take(pageSize).ToListAsync(ct);
    ...
    )
    {
        ...
        var q = _db.ShopProducts.Where(sp =>
            productIds.Contains(sp.ProductId) && sp.IsActive && !sp.IsDeleted
        );
    ...
    )
    {
        ...
        await _db.Database.ExecuteSqlInterpolatedAsync(
            ...
            WHERE ""Id"" = {shopProduct.Id} AND ""IsDeleted"" = false",
            ct
        );
