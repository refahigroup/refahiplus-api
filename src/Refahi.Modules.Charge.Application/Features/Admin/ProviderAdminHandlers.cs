        ...
        IRequestHandler<GetProviderWalletChargeReportQuery, ProviderReportDto>
{
    ...
    public ProviderAdminHandlers(IChargeProviderResolver providers) => _providers = providers;

    public Task<ProviderBalanceDto> Handle(GetProviderBalanceQuery q, CancellationToken ct) =>
        _providers.GetDefault().GetBalanceAsync(ct);

    public Task<IReadOnlyList<ProviderErrorDto>> Handle(
        GetProviderErrorsQuery q,
        CancellationToken ct
    ) => _providers.GetDefault().GetErrorsAsync(ct);

    public Task<IReadOnlyList<ProviderChannelDto>> Handle(
        GetProviderChannelsQuery q,
        CancellationToken ct
    ) => _providers.GetDefault().GetChannelsAsync(ct);

    public Task<ProviderReportDto> Handle(
        GetProviderTransactionReportQuery q,
        CancellationToken ct
    ) =>
        _providers
            .GetDefault()
    ...
    public Task<ProviderReportDto> Handle(
        GetProviderWalletChargeReportQuery q,
        CancellationToken ct
    ) =>
        _providers
            .GetDefault()
