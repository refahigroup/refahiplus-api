...
public sealed class GetProviderCallLogsHandler
    : IRequestHandler<GetProviderCallLogsQuery, IReadOnlyList<ProviderCallLogDto>>
{
    private readonly IChargeRequestRepository _requests;
    private readonly IProviderCallLogRepository _logs;

    public GetProviderCallLogsHandler(
        IChargeRequestRepository requests,
        IProviderCallLogRepository logs
    )
    {
        _requests = requests;
        _logs = logs;
    ...
    public async Task<IReadOnlyList<ProviderCallLogDto>> Handle(
        GetProviderCallLogsQuery query,
    ...
    )
    {
        ...
        var rows = await _logs.GetForChargeRequestAsync(
            query.RequestId,
            (page - 1) * size,
            size,
            ct
