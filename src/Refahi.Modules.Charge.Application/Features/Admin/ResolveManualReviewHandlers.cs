...
public sealed class ConfirmChargeFulfilledValidator
    : AbstractValidator<ConfirmChargeFulfilledCommand>
{
    public ConfirmChargeFulfilledValidator()
    {
        RuleFor(x => x.RequestId).NotEmpty().WithMessage("شناسه درخواست شارژ الزامی است");
        RuleFor(x => x.ProviderRrn)
            .NotEmpty()
            .MaximumLength(100)
            .WithMessage("شماره مرجع تامین‌کننده الزامی است");
        RuleFor(x => x.ProviderTraceId)
            .NotEmpty()
            .MaximumLength(150)
            .WithMessage("شناسه پیگیری تامین‌کننده الزامی است");
        RuleFor(x => x.Evidence)
            .NotEmpty()
            .MaximumLength(1000)
            .WithMessage("مستند تایید الزامی است");
    }
}
...
public sealed class RefundChargeRequestValidator : AbstractValidator<RefundChargeRequestCommand>
{
    public RefundChargeRequestValidator()
    {
        ...
        RuleFor(x => x.Reason)
            .NotEmpty()
            .MaximumLength(1000)
            .WithMessage("دلیل بازگشت وجه الزامی است");
...
public sealed class ConfirmChargeFulfilledHandler
    : IRequestHandler<ConfirmChargeFulfilledCommand, ReconcileChargeRequestResponse>
{
    ...
    public async Task<ReconcileChargeRequestResponse> Handle(
        ConfirmChargeFulfilledCommand command,
        CancellationToken ct
    )
    {
        var request =
            await _requests.GetAsync(command.RequestId, ct)
            ?? throw new ArgumentException("درخواست شارژ یافت نشد");
        request.ConfirmFulfilledByAdmin(
            command.ProviderRrn,
            command.ProviderTraceId,
            command.Evidence,
            DateTime.UtcNow
        );
...
public sealed class RefundChargeRequestHandler
    : IRequestHandler<RefundChargeRequestCommand, ReconcileChargeRequestResponse>
{
    private readonly IChargeRequestRepository _requests;
    private readonly ChargeRefundProcessor _refunds;

    public RefundChargeRequestHandler(
    ...
    public async Task<ReconcileChargeRequestResponse> Handle(
        RefundChargeRequestCommand command,
        CancellationToken ct
    )
    {
        var request =
