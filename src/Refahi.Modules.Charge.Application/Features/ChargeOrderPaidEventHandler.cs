...
public sealed class ChargeOrderPaidEventHandler : INotificationHandler<OrderPaidIntegrationEvent>
{
    ...
    public ChargeOrderPaidEventHandler(
        IChargeRequestRepository requests,
        ILogger<ChargeOrderPaidEventHandler> logger
    )
    {
        _requests = requests;
        _logger = logger;
    ...
    {
        bool isCharge =
            !e.SourceModule.Equals("Charge", StringComparison.OrdinalIgnoreCase)
        ...
        {
            _logger.LogWarning(
                "Charge paid event has no source reference. OrderId={OrderId}",
                e.OrderId
            );
            return;
        }
        ...
        {
            _logger.LogWarning(
                "Charge paid event did not match request ownership. OrderId={OrderId}, ChargeRequestId={ChargeRequestId}",
                e.OrderId,
                e.SourceReferenceId
            );
            return;
        ...
        _logger.LogInformation(
            "Charge request queued after order payment. ChargeRequestId={ChargeRequestId}, OrderId={OrderId}, SagaId={SagaId}",
            request.Id,
            e.OrderId,
            request.SagaId
        );
