...
public sealed class ReconcileChargeRequestHandler
    : IRequestHandler<ReconcileChargeRequestCommand, ReconcileChargeRequestResponse>
{
    private readonly IChargeRequestRepository _requests;
    private readonly ChargeFulfillmentProcessor _processor;

    public ReconcileChargeRequestHandler(
        IChargeRequestRepository requests,
        ChargeFulfillmentProcessor processor
    )
    {
        _requests = requests;
    ...
    public async Task<ReconcileChargeRequestResponse> Handle(
        ReconcileChargeRequestCommand c,
        CancellationToken ct
    )
    {
        var request =
            await _requests.GetAsync(c.RequestId, ct)
        ...
        if (request.Status == ChargeRequestStatus.ManualReview && !c.ForceManualReviewReset)
            throw new ArgumentException(
                "برای پردازش درخواست بررسی دستی باید گزینه اجباری فعال باشد"
            );
        ...
        return new(
            ...
            updated.Attempts.Count,
            updated.NextReconciliationAt,
            updated.ProviderRrn,
            updated.ProviderTraceId,
