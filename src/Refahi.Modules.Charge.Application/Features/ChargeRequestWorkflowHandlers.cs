...
public sealed class ConvertChargeRequestToOrderValidator
    : AbstractValidator<ConvertChargeRequestToOrderCommand>
{
...
public sealed class ConvertChargeRequestToOrderHandler
    : IRequestHandler<ConvertChargeRequestToOrderCommand, ConvertChargeRequestToOrderResponse>
{
    ...
    public async Task<ConvertChargeRequestToOrderResponse> Handle(
        ConvertChargeRequestToOrderCommand command,
        CancellationToken ct
    )
    {
        var request =
            await _requests.GetForUserAsync(command.RequestId, command.UserId, ct)
    ...
    private static string ServiceCaption(ChargeServiceType type) =>
        type switch
        {
    ...
    private static string OperatorCaption(ChargeOperator op) =>
        op switch
        {
    ...
    private static string CategoryCode(ChargeServiceType type) =>
        $"mobile-charge.{type switch
    {
