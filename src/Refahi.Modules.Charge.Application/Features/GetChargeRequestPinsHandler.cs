...
public sealed class GetChargeRequestPinsHandler
    : IRequestHandler<GetChargeRequestPinsQuery, IReadOnlyList<ChargePinDeliveryDto>?>
{
    private readonly IChargeRequestRepository _requests;
    private readonly IChargeSecretProtector _protector;

    public GetChargeRequestPinsHandler(
    ...
    public async Task<IReadOnlyList<ChargePinDeliveryDto>?> Handle(
        GetChargeRequestPinsQuery query,
    ...
    )
    {
        ...
        if (
            ...
            || request.Status != ChargeRequestStatus.Fulfilled
        )
            throw new ArgumentException("پین‌های این درخواست هنوز قابل دریافت نیستند");

        return request
            .Pins.Select(x => new ChargePinDeliveryDto(
