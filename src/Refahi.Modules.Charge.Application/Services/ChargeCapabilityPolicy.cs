...
public static class ChargeCapabilityPolicy
{
    ...
    public static ChargeServiceCapabilityDto Get(
        ChargeOperator @operator,
        ChargeServiceType service
    )
    {
        var supported = service switch
        {
            ChargeServiceType.PostpaidBill or ChargeServiceType.CreditLimit => @operator
                == ChargeOperator.Irancell,
            ChargeServiceType.PinCharge => @operator
                is ChargeOperator.Irancell
                    or ChargeOperator.Mci
                    or ChargeOperator.Rightel,
            _ => true,
        };
        ...
        IReadOnlyList<long> suggestions = service
            is ChargeServiceType.DirectCharge
                or ChargeServiceType.CreditLimit
