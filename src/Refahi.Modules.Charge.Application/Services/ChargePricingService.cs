...
public sealed class ChargePricingService
{
    ...
    public async Task<ChargePriceResult> CalculateAsync(ChargeOperator @operator, ChargeServiceType serviceType,
        long providerCostMinor, DateTime nowUtc, CancellationToken ct)
    {
        var rule = await _rules.FindApplicableAsync(@operator, serviceType, nowUtc, ct);
        if (rule is null) return new(null, 0, 0, 0, providerCostMinor);
        ...
        return new(rule.Id, rule.Percent, rule.FixedAmountMinor, markup, checked(providerCostMinor + markup));
    }
}
