...
public class PaymentCallbackEndpoint : IEndpoint
{
        ...
        routes
            .MapPost(
                ...
                ) =>
                {
                    // Parse provider type
                    if (
                        !Enum.TryParse<PaymentGatewayProviderType>(
                            provider,
                            ignoreCase: true,
                    ...
                    var rawJson = JsonSerializer.Serialize(
                        form.ToDictionary(kvp => kvp.Key, kvp => kvp.Value.ToString())
                    );
                    ...
                    var command = new ProcessCallbackCommand(
                        ...
                        HashedCardNumber: string.IsNullOrWhiteSpace(hashedCardNumber)
                            ? null
                            : hashedCardNumber,
    ...
    private static string ReadFormValue(IFormCollection form, string key)
    {
        ...
        var fallback = form.FirstOrDefault(item =>
