...
public sealed record ConvertChargeRequestToOrderCommand(
    Guid RequestId,
    Guid UserId,
    string IdempotencyKey
) : IRequest<ConvertChargeRequestToOrderResponse>;

public sealed record ConvertChargeRequestToOrderResponse(
    Guid RequestId,
    Guid OrderId,
    string OrderNumber,
    long FinalAmountMinor,
...
public sealed record ChargeServiceCapabilityDto(
    ...
    IReadOnlyList<long> SuggestedAmountsMinor
);

public sealed record ChargeOperatorDto(
    short Id,
    string Code,
    string PersianName,
...
public sealed record GetProductsQuery(ChargeOperator Operator)
    : IRequest<IReadOnlyList<Providers.ChargeProductDto>>;

public sealed record GetOffersQuery(
    ChargeOperator Operator,
    string MobileNumber,
    Providers.ChargeOfferCategory Category
) : IRequest<IReadOnlyList<Providers.ChargeProductDto>>;
...
public sealed record GetPinCategoriesQuery(ChargeOperator? Operator = null)
    : IRequest<IReadOnlyList<Providers.PinChargeCategoryDto>>;

public sealed record GetPackageTypesQuery : IRequest<IReadOnlyList<Providers.PackageTypeDto>>;

public sealed record ReconcileChargeRequestCommand(
    Guid RequestId,
    bool ForceManualReviewReset = false
) : IRequest<ReconcileChargeRequestResponse>;

public sealed record ReconcileChargeRequestResponse(
    Guid RequestId,
    string Status,
    int AttemptCount,
    DateTime? NextReconciliationAt,
    string? ProviderRrn,
    string? ProviderTraceId,
    string? Message
);
