...
public sealed record UpsertMarkupRuleCommand(
    Guid? RuleId,
...
public sealed record MarkupRuleDto(
    Guid Id,
    ChargeOperator? Operator,
    ChargeServiceType? ServiceType,
    decimal Percent,
...
public sealed record GetProviderChannelsQuery
    : IRequest<IReadOnlyList<Providers.ProviderChannelDto>>;

public sealed record GetProviderTransactionReportQuery(
    int PageNumber,
    DateOnly? FromDate,
    DateOnly? ToDate
) : IRequest<Providers.ProviderReportDto>;

public sealed record GetProviderWalletChargeReportQuery(
    int PageNumber,
    DateOnly? FromDate,
    DateOnly? ToDate
) : IRequest<Providers.ProviderReportDto>;
