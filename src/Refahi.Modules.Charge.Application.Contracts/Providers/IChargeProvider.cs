...
{
    string Name { get; }
    Task<IReadOnlyList<ChargeProductDto>> GetProductsAsync(
        ChargeOperator @operator,
        CancellationToken ct
    );
    Task<IReadOnlyList<ChargeProductDto>> GetOffersAsync(
        ChargeOperator @operator,
        string mobileNumber,
        ChargeOfferCategory category,
        CancellationToken ct
    );
    Task<ChargeEligibilityDto> CheckEligibilityAsync(
        ChargeEligibilityRequest request,
        CancellationToken ct
    );
    Task<ChargePostpaidBalanceDto> GetPostpaidBalanceAsync(
        ChargeOperator @operator,
        string mobileNumber,
        CancellationToken ct
    );
    ...
    Task<ProviderPurchaseResultDto> PurchaseAsync(
    ...
    Task<ProviderReportDto> GetTransactionReportAsync(
        ProviderReportRequest request,
        CancellationToken ct
    );
    Task<ProviderReportDto> GetWalletChargeReportAsync(
        ProviderReportRequest request,
        CancellationToken ct
    );
}
...
public sealed record ChargeEligibilityDto(
    bool Supported,
    bool? Eligible,
    string? LineType,
    int EniacResultCode,
...
public sealed record ChargePostpaidBalanceDto(
    string CorrelationId,
    long CurrentBalanceMinor,
    long? OutstandingBalanceMinor,
    int ResultCode,
...
public sealed record PackageTypeDto(
    string Title,
    string? PersianTitle,
    int PackageTypeCode,
    ChargeOperator Operator,
...
public sealed record ProviderTraceRequest(
    long EniacRrn,
    string CustomerInvoiceNumber,
    string ProviderProductId,
    long AmountMinor,
...
public sealed record ProviderBalanceDto(
    long TotalBalanceMinor,
    long SuspendedBalanceMinor,
