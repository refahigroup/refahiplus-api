...
public sealed record VoucherDto(
    Guid Id, Guid StoreOrderId, Guid StoreOrderItemId, int SequenceNumber,
    Guid OrderId, string OrderNumber,
    Guid SupplierId, string SupplierName, Guid ShopId, string ShopName,
    Guid ProductId, string ProductTitle, string Code, string Status,
    DateTimeOffset IssuedAtUtc, DateTimeOffset? RedeemedAtUtc,
    Guid? RedeemedShopId, string? RedeemedShopName,
    DateTimeOffset? RevokedAtUtc, DateTimeOffset? ExpiresAtUtc);

public sealed record VoucherAuditDto(
    Guid Id, Guid StoreOrderId, Guid StoreOrderItemId, int SequenceNumber,
    Guid OrderId, string OrderNumber, Guid UserId,
    Guid SupplierId, string SupplierName, Guid ShopId, string ShopName,
    Guid ProductId, string ProductTitle, string Status,
    DateTimeOffset IssuedAtUtc, DateTimeOffset? RedeemedAtUtc,
    Guid? RedeemedByUserId, Guid? RedeemedShopId, string? RedeemedShopName,
    DateTimeOffset? RevokedAtUtc, string? RevocationReason, DateTimeOffset? ExpiresAtUtc);

public sealed record VoucherRedemptionDto(
    Guid VoucherId, Guid StoreOrderId, Guid ProductId, string ProductTitle,
    Guid VendorUserId, Guid SupplierId, Guid ShopId, DateTimeOffset RedeemedAtUtc);

public sealed record VoucherRedemptionHistoryItemDto(
    Guid VoucherId, string MaskedReference, Guid StoreOrderId,
...
public sealed record VoucherRedemptionHistoryPageDto(
    int Page, int PageSize, int Total, IReadOnlyList<VoucherRedemptionHistoryItemDto> Items);

public sealed record RedeemVoucherCommand(
    Guid VendorUserId, Guid ShopId, string Code, string IdempotencyKey)
...
public sealed record GetVendorVoucherRedemptionHistoryQuery(
    Guid VendorUserId, Guid SupplierId, Guid? ShopId, int Page = 1, int PageSize = 20)
...
public sealed record PrepareStoreOrderRefundCommand(Guid OrderId, string Reason, Guid? VoucherRefundOverrideId = null)
...
public sealed record AdminStoreOrderRefundDto(
    Guid StoreOrderId, Guid OrderId, string StoreOrderStatus, long FinalAmountMinor,
    bool RefundBlockedByRedeemedVoucher, string? RefundBlockerCode,
...
public sealed record VoucherRefundOverrideDto(
    Guid Id, Guid StoreOrderId, Guid OrderId, Guid AdminUserId, string Reason,
    IReadOnlyList<VoucherRefundSnapshotItemDto> VoucherSnapshot,
    DateTimeOffset CreatedAtUtc, Guid CorrelationId, string Outcome,
...
public sealed record VoucherRefundOverrideAttemptDto(
    int SequenceNumber, string Outcome, string? PaymentAction,
...
public sealed record OverrideRedeemedVoucherRefundCommand(
    Guid OrderId, Guid AdminUserId, string Reason, string IdempotencyKey)
    : IRequest<VoucherRefundOverrideDto>;
public sealed record VoucherErrorResponse(bool Success, string Code, string Message, int StatusCode);
...
{
    public VoucherApplicationException(string code, string message) : base(message) => Code = code;
