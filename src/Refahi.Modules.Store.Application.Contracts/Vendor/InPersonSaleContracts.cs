...
public sealed record InPersonOrderDto(
    ...
    Guid? StoreOrderId = null, string? CheckoutDestination = null);

public sealed record InPersonProductDto(
    Guid ProductId, string Title, string? CategoryName, Guid AgreementProductId,
    int CategoryId = 0, Guid? AgreementId = null, Guid? AgreementCategoryTermId = null,
...
public sealed record VendorStoreOrderItemSnapshotDto(
    Guid StoreOrderItemId, Guid ProductId, Guid? OfferId, string ProductTitle,
    int CategoryId, string CategoryCode, Guid SupplierId, Guid ShopId,
    string SalesChannel, string ProductType, string SalesModel, string FulfillmentMethod,
    int Quantity, long UnitPriceMinor, long GrossAmountMinor, long? DeclaredGrossAmountMinor,
    Guid AgreementId, Guid AgreementCategoryTermId, decimal CommissionPercent,
    long CommissionAmountMinor);

public sealed record VendorStoreOrderSnapshotDto(
    Guid StoreOrderId, Guid OrderId, Guid UserId, Guid CreatedByUserId,
    string SalesChannel, string InitiatorType, string Status,
    Guid SupplierId, Guid ShopId, long FinalAmountMinor,
    long? DeclaredGrossAmountMinor, IReadOnlyList<VendorStoreOrderItemSnapshotDto> Items);
...
public sealed record GetVendorStoreOrdersByOrderIdsQuery(
    Guid VendorUserId, IReadOnlyList<Guid> OrderIds)
    : IRequest<IReadOnlyList<VendorStoreOrderSnapshotDto>>;

public sealed record GetUserStoreOrderByOrderIdQuery(Guid UserId, Guid OrderId, bool IsAdmin = false)
    : IRequest<VendorStoreOrderSnapshotDto?>;

public sealed record GetUserStoreOrdersByOrderIdsQuery(
    Guid UserId, IReadOnlyList<Guid> OrderIds, bool IsAdmin = false)
    : IRequest<IReadOnlyList<VendorStoreOrderSnapshotDto>>;
...
public sealed record StartUserInPersonOrderCommand(
    Guid UserId, Guid ShopId, Guid ProductId, long AmountMinor,
    string IdempotencyKey) : IRequest<InPersonOrderDto>;
