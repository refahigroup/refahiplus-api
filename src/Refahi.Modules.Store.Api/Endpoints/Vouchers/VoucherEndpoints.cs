...
public sealed class VoucherEndpoints : IEndpoint
{
    public void Map(object app)
    {
                ...
                async (ClaimsPrincipal principal, IMediator mediator, CancellationToken ct) =>
                {
                    ...
                    return Results.Ok(
                        ApiResponseHelper.Success(
                            await mediator.Send(new GetMyVouchersQuery(userId), ct)
                        )
                    );
                }
            )
                    ...
                    return result is null
                        ? Results.NotFound(
                            new VoucherErrorResponse(
                                false,
                                "VOUCHER_NOT_FOUND",
                                "ووچر یافت نشد یا متعلق به شما نیست",
                                404
                            )
                        )
        ...
        routes
            .MapPost(
                ...
                ) =>
                {
                    ...
                    return Results.Ok(
                        ApiResponseHelper.Success(result, "ووچر با موفقیت استفاده شد")
                    );
                }
            )
            .WithName("Store.Vendor.Vouchers.Redeem")
            .WithTags("Store.Vendor.Vouchers")
            .RequireAuthorization("VendorOnly")
            .RequireRateLimiting("VoucherRedeem")
            .AddEndpointFilter<VoucherTypedErrorFilter>()
        ...
        routes
            .MapGet(
                "/vendor/vouchers/history",
                async (
                    [AsParameters] VendorVoucherHistoryRequest query,
                    ClaimsPrincipal principal,
                    IMediator mediator,
                    CancellationToken ct
                ) =>
            ...
            .WithName("Store.Vendor.Vouchers.History")
            .WithTags("Store.Vendor.Vouchers")
            .RequireAuthorization("VendorOnly")
            .RequireRateLimiting("VoucherRedeem")
            .AddEndpointFilter<VoucherTypedErrorFilter>()
            ...
            .MapGet(
                ...
                async (
                    ...
                    Results.Ok(
                        ApiResponseHelper.Success(
                            await mediator.Send(
                                new GetAdminVoucherAuditQuery(storeOrderId, voucherId),
                                ct
                            )
                        )
                    )
            )
            .WithName("Store.Admin.Vouchers.Audit")
            .WithTags("Store.Admin.Vouchers")
        ...
        routes
            .MapGet(
                "/admin/store-orders/by-order/{orderId:guid}/refund",
                async (Guid orderId, IMediator mediator, CancellationToken ct) =>
                {
                    var result = await mediator.Send(
                        new GetAdminStoreOrderRefundQuery(orderId),
                        ct
                    );
                    return result is null
                        ? Results.NotFound(
            ...
            .WithName("Store.Admin.StoreOrders.RefundDetail")
        ...
        routes
            .MapPost(
                "/admin/store-orders/by-order/{orderId:guid}/voucher-refund-override",
                async (
                    Guid orderId,
                    OverrideRedeemedVoucherRefundRequest body,
                    ClaimsPrincipal principal,
                    IMediator mediator,
                    CancellationToken ct
                ) =>
                {
                    if (!TryUser(principal, out var adminUserId))
                        return Results.Unauthorized();
                    var result = await mediator.Send(
                        new OverrideRedeemedVoucherRefundCommand(
                            orderId,
                            adminUserId,
                            body.Reason,
                            body.IdempotencyKey
