    ...
    public void Map(object app)
    {
        ...
        routes
            .MapGet(
                ...
                {
                    if (
                        string.IsNullOrWhiteSpace(ownerType) || string.IsNullOrWhiteSpace(ownerSlug)
                    )
                        return Results.BadRequest(
                            ApiResponseHelper.Error("نوع مالک و شناسه‌ی آن الزامی است")
                        );
                    ...
                    var result = await mediator.Send(
                        new GetBannersQuery(parsedType, ownerId, type),
                        ct
                    );
