        ...
        routes
            .MapGet(
                ...
                async (ClaimsPrincipal principal, IMediator mediator, CancellationToken ct) =>
                {
                    ...
                    foreach (
                        var context in contexts.Where(x =>
                            x.Permissions.Contains(StorePermissions.ViewIncomeWallet)
                        )
                    )
                    {
                        var wallet = (
                            await mediator.Send(new GetMyWalletsQuery(context.VendorId), ct)
                        ).SingleOrDefault(x =>
            ...
            .WithName("Store.Vendor.IncomeWallets")
            .WithTags("Store.Vendor.Wallet")
            .RequireAuthorization("VendorOnly");

        routes
            .MapGet(
                "/vendor/income-wallet-transactions",
                async (
                    int? take,
            ...
            .WithName("Store.Vendor.IncomeWalletTransactions")
            .WithTags("Store.Vendor.Wallet")
            .RequireAuthorization("VendorOnly");
    }
