...
public class DeactivateDailyDealEndpoint : IEndpoint
{
    public void Map(object app)
    {
        ...
        routes
            .MapPost(
                "/admin/daily-deals/{dealId:int}/deactivate",
                async (int dealId, IMediator mediator, CancellationToken ct) =>
                {
                    var command = new DeactivateDailyDealCommand(dealId);
                    var result = await mediator.Send(command, ct);
                    return Results.Ok(
                        ApiResponseHelper.Success(result, "پیشنهاد ویژه با موفقیت غیرفعال شد")
                    );
                }
            )
