...
) : IEndpointFilter
{
    ...
    )
    {
        ...
        if (
            http.Request.Path.Value?.Contains("/provider/", StringComparison.OrdinalIgnoreCase)
                != true
        ...
        if (
            !supplierId.HasValue
            || !await mediator.Send(
                new AuthorizeStoreResourceQuery(userId, supplierId.Value, null, permission),
                http.RequestAborted
            )
        )
            return Results.Json(
    ...
    )
    {
        ...
        if (
            !productId.HasValue
            && http.Request.Path.Value?.Contains(
                "/provider/products/",
        ...
        )
        {
            ...
            var product = session is null
                ? null
                : await products.GetByIdAsync(session.ProductId, ct);
            if (product is not null)
                return product.SupplierId != Guid.Empty
                    ? product.SupplierId
                    : await SupplierByAgreementProductAsync(product.AgreementProductId, ct);
    ...
    private async Task<Guid?> SupplierByAgreementProductAsync(
    ...
    )
    {
        ...
        if (product is null)
            return null;
        return (
            await mediator.Send(new GetAgreementByIdQuery(product.AgreementId), ct)
        )?.SupplierId;
    }
