...
public sealed class VoucherTypedErrorFilter : IEndpointFilter
{
    public async ValueTask<object?> InvokeAsync(EndpointFilterInvocationContext context, EndpointFilterDelegate next)
    {
        ...
        catch (VoucherApplicationException ex)
        {
            ...
            return Results.Json(new VoucherErrorResponse(false, ex.Code, ex.Message, status), statusCode: status);
        }
        catch (StoreConcurrencyException)
        {
            const int status = StatusCodes.Status409Conflict;
            return Results.Json(new VoucherErrorResponse(false, "VOUCHER_CONCURRENCY_CONFLICT",
                "وضعیت ووچر هم‌زمان تغییر کرده است؛ دوباره تلاش کنید", status), statusCode: status);
        }
        catch (StoreDomainException ex)
        {
            const int status = StatusCodes.Status400BadRequest;
            return Results.Json(new VoucherErrorResponse(false, ex.ErrorCode, ex.Message, status), statusCode: status);
        }
    }
}
