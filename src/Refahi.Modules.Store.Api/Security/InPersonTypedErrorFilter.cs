...
public sealed class InPersonTypedErrorFilter : IEndpointFilter
{
    public async ValueTask<object?> InvokeAsync(EndpointFilterInvocationContext context, EndpointFilterDelegate next)
    {
        ...
        {
            var status = ex.ErrorCode.EndsWith("_NOT_FOUND", StringComparison.Ordinal) ||
            ...
            return Results.Json(new InPersonErrorResponse(false, ex.ErrorCode, ex.Message, status), statusCode: status);
        }
        catch (UnauthorizedAccessException ex)
        {
            return Results.Json(new InPersonErrorResponse(false, "IN_PERSON_FORBIDDEN", ex.Message,
                StatusCodes.Status403Forbidden), statusCode: StatusCodes.Status403Forbidden);
        }
    }
}

public sealed record InPersonErrorResponse(bool Success, string Code, string Message, int StatusCode);
