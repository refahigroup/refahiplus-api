...
public sealed class UpdateAgreementCategoryTermEndpoint : IEndpoint
{
    public void Map(object app)
    {
        ...
        routes.MapPut("/admin/agreements/{agreementId:guid}/agreement-category-terms/{termId:guid}", async (
            Guid agreementId,
            Guid termId,
        ...
        {
            await mediator.Send(new UpdateAgreementCategoryTermCommand(
                agreementId,
                termId,
                body.CategoryId,
                body.AllowedSalesChannels,
                body.CommissionPercent), cancellationToken);
            return Results.Ok(ApiResponseHelper.Success<object?>(null, "شرط دسته‌بندی قرارداد با موفقیت به‌روزرسانی شد"));
        })
