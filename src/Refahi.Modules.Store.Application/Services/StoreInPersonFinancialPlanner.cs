...
public sealed class StoreInPersonFinancialPlanner(
    IMediator mediator,
    IOptions<StorePaymentDistributionOptions> options) : IStoreInPersonFinancialPlanner
{
        ...
        bool vatApplicable, CancellationToken ct)
    {
        ...
        return new StoreInPersonFinancialPlan(grossAmountMinor, commissionPercent, commission,
            vatPercent, vat, net, providerWallet.WalletId, postings);
    }

    private async Task<WalletInfoDto> RequireSystemWalletAsync(Guid walletId, CancellationToken ct)
    {
        var wallet = await mediator.Send(new GetWalletInfoByIdQuery(walletId), ct)
            ?? throw new StoreDomainException("کیف سیستمی پرداخت حضوری یافت نشد", "SYSTEM_WALLET_NOT_FOUND");
        if (wallet.WalletType != 1 || wallet.Status != 1 || wallet.Currency != "IRR")
            throw new StoreDomainException("کیف سیستمی پرداخت حضوری معتبر یا فعال نیست", "INVALID_SYSTEM_WALLET");
        return wallet;
    }
