...
public sealed class GetOfferCartQueryHandler(
    ...
    public async Task<OfferCartDto?> Handle(GetOfferCartQuery request, CancellationToken ct)
    {
        ...
        foreach (var item in cart.Items.Where(x => x.OfferId.HasValue))
        {
            ...
            var availabilityCode =
                available ? "AVAILABLE"
                : current is null ? "OFFER_NOT_EFFECTIVE"
                : product is null ? "PRODUCT_NOT_FOUND"
                : "SHOP_NOT_FOUND";
            var availabilityReason =
                available ? "قابل خرید"
                : current is null ? "پیشنهاد منقضی یا در دسترس نیست"
                : product is null ? "محصول یافت نشد"
                : "فروشگاه یافت نشد";
            if (available)
            {
                try
                {
                    _ = await eligibility.ResolveByIdAsync(
                        current!.Id,
                        item.Quantity,
                        item.VariantId,
                        item.SessionId,
                        ...
                        .Combinations.Select(combination =>
                        {
                            ...
                            return attribute
                                ?.Values.FirstOrDefault(x =>
                                    x.Id == combination.VariantAttributeValueId
                                )
                                ?.Value;
                        })
            ...
            var image =
                variant?.ImageUrl
                ?? product
                    ?.Images.OrderByDescending(x => x.IsMain)
                    .ThenBy(x => x.SortOrder)
                    .FirstOrDefault()
                    ?.ImageUrl;
            result.Add(
                new OfferCartItemDto(
                    item.Id,
                    item.OfferId!.Value,
                    item.ProductId,
                    item.ShopId,
                    ...
                    variantLabel,
                    sessionLabel,
                    product?.FulfillmentMethod.ToString() ?? string.Empty,
        ...
        return new OfferCartDto(
            cart.Id,
            result,
            result.Sum(x => checked(x.SnapshotFinalUnitPriceMinor * x.Quantity)),
            result.Sum(x =>
                checked(
                    (x.CurrentFinalUnitPriceMinor ?? x.SnapshotFinalUnitPriceMinor) * x.Quantity
                )
            ),
            result.Sum(x => x.Quantity),
            result.Any(x => x.HasOfferChanged)
        );
    }
}
