...
{
    public async Task<AddOfferToCartResponse> Handle(
        AddOfferToCartCommand request,
    ...
    )
    {
        ...
        var cart = await carts.AddOfferItemAsync(
            request.UserId,
            request.ModuleId,
            context.Shop.Id,
            context.Product.Id,
        ...
        var projection =
            await mediator.Send(new GetOfferCartQuery(request.UserId, request.ModuleId), ct)
