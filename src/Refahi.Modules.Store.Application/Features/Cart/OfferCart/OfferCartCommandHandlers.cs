...
namespace Refahi.Modules.Store.Application.Features.Cart.OfferCart;
...
) : IRequestHandler<UpdateOfferCartItemCommand, OfferCartDto>
{
    ...
    {
        var cart = await OfferCartSupport.GetOwnedCartAsync(
            carts,
            request.UserId,
            request.ModuleId,
            ct
        ...
        var item =
            cart.Items.FirstOrDefault(x => x.Id == request.CartItemId)
        ...
        var changed = OfferCartSupport.HasChanged(item, current);
        if (changed && !request.AcceptOfferChanges)
            throw OfferCartSupport.Changed(item, current);
        if (current is null)
            throw OfferCartSupport.Changed(item, null);

        var context = await eligibility.ResolveByIdAsync(
            current.Id,
            request.Quantity,
            item.VariantId,
            item.SessionId,
        ...
        return await OfferCartSupport.ProjectAsync(
            mediator,
            request.UserId,
            request.ModuleId,
            ct
...
) : IRequestHandler<ReconfirmOfferCartCommand, OfferCartDto>
{
    public async Task<OfferCartDto> Handle(ReconfirmOfferCartCommand request, CancellationToken ct)
    {
        ...
        foreach (var item in cart.Items)
        {
            ...
            if (current is null)
            {
                unavailable.Add(OfferCartSupport.Changed(item, null).Details.Single());
                continue;
            }
            var context = await eligibility.ResolveByIdAsync(
                current.Id,
        ...
        if (unavailable.Count > 0)
            throw new CartOfferChangedException(unavailable);
...
) : IRequestHandler<SyncOfferCartCommand, OfferCartDto>
{
    ...
    {
        var cacheKey =
            $"sync_offer_cart_v3:{request.UserId:N}:{request.ModuleId}:{request.IdempotencyKey}";
        ...
        try
        {
            if (cache.TryGetValue<SyncCacheEntry>(cacheKey, out var cached) && cached is not null)
            {
                if (
                    !CryptographicOperations.FixedTimeEquals(
                        Encoding.ASCII.GetBytes(cached.Fingerprint),
                        Encoding.ASCII.GetBytes(fingerprint)
                    )
                )
                    throw new StoreDomainException(
                        "کلید همگام‌سازی قبلاً با محتوای دیگری استفاده شده است",
            ...
            var grouped = request
                .Items.GroupBy(x => new
                {
                    ...
                    x.ProductSessionId,
                    x.UsageDate,
                    x.SnapshotOriginalUnitPriceMinor,
                    x.SnapshotFinalUnitPriceMinor,
                })
            ...
            foreach (var tuple in grouped)
            {
                ...
                Offer? current = referenced is null
                    ? null
                ...
                var changed =
                    current is null
                    || current.Id != input.OfferId
                    || current.OriginalPriceMinor != input.SnapshotOriginalUnitPriceMinor
                    || current.FinalPriceMinor != input.SnapshotFinalUnitPriceMinor;
                if (changed && !request.AcceptOfferChanges)
                {
                    drifts.Add(
                        new CartOfferChangedDetail(
                            null,
                            input.OfferId,
                            input.SnapshotOriginalUnitPriceMinor,
                            input.SnapshotFinalUnitPriceMinor,
                            current?.Id,
                            current?.OriginalPriceMinor,
                            current?.FinalPriceMinor,
                            current is null
                                ? "پیشنهاد منقضی یا ناموجود است"
                                : "پیشنهاد یا قیمت تغییر کرده است"
                        )
                    );
                    continue;
                }
            ...
            if (drifts.Count > 0)
                throw new CartOfferChangedException(drifts);
            var shops = resolved
                .Select(x => x.Shop.Id)
    ...
    private static string Fingerprint(SyncOfferCartCommand request)
    {
        var canonical = new StringBuilder("store-cart-sync-v3|")
            ...
            .Append('|')
            .Append(request.AcceptOfferChanges ? '1' : '0');
        foreach (
            var item in request
                .Items.OrderBy(x => x.OfferId)
                .ThenBy(x => x.ProductVariantId)
                .ThenBy(x => x.ProductSessionId)
                .ThenBy(x => x.UsageDate)
        )
            canonical
                .Append('|')
                .Append(item.OfferId.ToString("N"))
                .Append(':')
                .Append(item.Quantity)
                .Append(':')
                .Append(item.ProductVariantId?.ToString("N") ?? "-")
                .Append(':')
                .Append(item.ProductSessionId?.ToString("N") ?? "-")
                .Append(':')
                .Append(item.UsageDate?.ToString("yyyy-MM-dd") ?? "-")
                .Append(':')
                .Append(item.SnapshotOriginalUnitPriceMinor)
                .Append(':')
                .Append(item.SnapshotFinalUnitPriceMinor);
...
internal static class OfferCartSupport
{
    public static async Task<CartAggregate> GetOwnedCartAsync(
        ICartRepository carts,
        Guid userId,
        int moduleId,
        CancellationToken ct
    ) =>
        await carts.GetByUserAndModuleIdAsync(userId, moduleId, ct)
        ?? throw new StoreDomainException("سبد خرید یافت نشد", "CART_NOT_FOUND");

    public static bool HasChanged(CartItem item, Offer? current) =>
        current is null
        || current.Id != item.OfferId
        || current.OriginalPriceMinor != item.OriginalUnitPriceMinor
        || current.FinalPriceMinor != item.UnitPriceMinor;

    public static CartOfferChangedException Changed(CartItem item, Offer? current) =>
        new([
            new CartOfferChangedDetail(
                item.Id,
                item.OfferId!.Value,
    ...
    public static async Task<OfferCartDto> ProjectAsync(
        IMediator mediator,
        Guid userId,
        int moduleId,
        CancellationToken ct
    ) =>
        await mediator.Send(new GetOfferCartQuery(userId, moduleId), ct)
        ?? throw new StoreDomainException("سبد خرید یافت نشد", "CART_NOT_FOUND");
}
