...
public class GetShopProductsQueryHandler
    : IRequestHandler<GetShopProductsQuery, ShopProductsPagedResponse>
{
    ...
    public async Task<ShopProductsPagedResponse> Handle(
    ...
    )
    {
        ...
        var agreementProductIds = productMap
            .Values.Select(p => p.AgreementProductId)
            .Distinct()
            .ToList();

        var agreementProducts =
            agreementProductIds.Count > 0
        ...
        var dtos = items
            .Select(sp =>
            {
                ...
                var commissionPrice = (long)
                    Math.Round(sp.DiscountedPrice * commissionPercent / 100m);
        ...
        return new ShopProductsPagedResponse(
            dtos,
            request.PageNumber,
            request.PageSize,
            total,
