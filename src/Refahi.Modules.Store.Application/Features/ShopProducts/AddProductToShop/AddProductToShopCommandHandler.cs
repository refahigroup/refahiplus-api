...
public class AddProductToShopCommandHandler
    : IRequestHandler<AddProductToShopCommand, AddProductToShopResponse>
{
    ...
    public async Task<AddProductToShopResponse> Handle(
        AddProductToShopCommand request,
        CancellationToken cancellationToken
    )
    {
        var shop =
            await _shopRepo.GetByIdAsync(request.ShopId, cancellationToken)
        ...
        if (isManual)
        {
            if (
                ...
                throw new StoreDomainException(
                    "محصول حضوری با فروشگاه انتخاب‌شده سازگار نیست",
                    "INVALID_MANUAL_SHOP_PRODUCT"
                );
        ...
        var existing = await _shopProductRepo.GetAsync(
            request.ShopId,
            request.ProductId,
            cancellationToken
        );
        if (existing is not null && !existing.IsDeleted)
            throw new StoreDomainException(
                "این محصول قبلاً به فروشگاه اضافه شده است",
                "SHOP_PRODUCT_EXISTS"
            );

        var shopProduct = isManual
            ? ShopProduct.CreateWithManualPricing(
                request.ShopId,
                request.ProductId,
                request.Description
            )
