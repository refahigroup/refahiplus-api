...
public class ActivateDailyDealCommandHandler
    : IRequestHandler<ActivateDailyDealCommand, ActivateDailyDealResponse>
{
    private readonly IDailyDealRepository _dealRepo;

    public ActivateDailyDealCommandHandler(IDailyDealRepository dealRepo) => _dealRepo = dealRepo;

    public async Task<ActivateDailyDealResponse> Handle(
        ActivateDailyDealCommand request,
        CancellationToken cancellationToken
    )
    {
        var deal =
            await _dealRepo.GetByIdAsync(request.Id, cancellationToken)
