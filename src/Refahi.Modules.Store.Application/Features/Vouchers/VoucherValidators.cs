...
public sealed class PrepareStoreOrderRefundValidator
    : AbstractValidator<PrepareStoreOrderRefundCommand>
{
...
public sealed class GetAdminStoreOrderRefundValidator
    : AbstractValidator<GetAdminStoreOrderRefundQuery>
{
    public GetAdminStoreOrderRefundValidator() =>
        RuleFor(x => x.OrderId).NotEmpty().WithMessage("شناسه سفارش الزامی است");
}
    ...
    : AbstractValidator<OverrideRedeemedVoucherRefundCommand>
{
    public OverrideRedeemedVoucherRefundValidator()
    {
        ...
        RuleFor(x => x.Reason)
            .Must(x => x is not null && x.Trim().Length >= 10)
