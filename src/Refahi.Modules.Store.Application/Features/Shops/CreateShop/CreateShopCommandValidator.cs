...
public class CreateShopCommandValidator : AbstractValidator<CreateShopCommand>
{
    public CreateShopCommandValidator()
    {
        RuleFor(x => x.Name)
            ...
            .MaximumLength(200)
            .WithMessage("نام فروشگاه نمی‌تواند بیشتر از ۲۰۰ کاراکتر باشد");

        RuleFor(x => x.Slug)
            .NotEmpty()
            .WithMessage("اسلاگ الزامی است")
            .MaximumLength(200)
            .WithMessage("اسلاگ نمی‌تواند بیشتر از ۲۰۰ کاراکتر باشد")
            .Matches(@"^[a-z0-9-]+$")
            .WithMessage("اسلاگ فقط می‌تواند شامل حروف کوچک انگلیسی، اعداد و خط تیره باشد");
        ...
        RuleFor(x => x.ManagerName)
            .MaximumLength(200)
            .WithMessage("نام مدیر نمی‌تواند بیشتر از ۲۰۰ کاراکتر باشد");

        RuleFor(x => x.ManagerPhone)
            .MaximumLength(20)
            .WithMessage("تلفن مدیر نمی‌تواند بیشتر از ۲۰ کاراکتر باشد")
            .Matches(@"^[0-9+\-()\s]*$")
        ...
        RuleFor(x => x.RepresentativeName)
            .MaximumLength(200)
            .WithMessage("نام نماینده نمی‌تواند بیشتر از ۲۰۰ کاراکتر باشد");

        RuleFor(x => x.RepresentativePhone)
            .MaximumLength(20)
            .WithMessage("تلفن نماینده نمی‌تواند بیشتر از ۲۰ کاراکتر باشد")
            .Matches(@"^[0-9+\-()\s]*$")
            .When(x => !string.IsNullOrEmpty(x.RepresentativePhone))
            .WithMessage("فرمت تلفن نماینده نامعتبر است");

        RuleFor(x => x.ContactPhone)
            .MaximumLength(20)
            .WithMessage("تلفن تماس نمی‌تواند بیشتر از ۲۰ کاراکتر باشد")
            .Matches(@"^[0-9+\-()\s]*$")
