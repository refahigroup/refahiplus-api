...
public sealed class GetSyntheticOffersV2QueryValidator
    : AbstractValidator<GetSyntheticOffersV2Query>
{
    private static readonly string[] OfferKinds =
    [
        "StockProduct",
        "StockVariant",
        "ProductSession",
    ...
    public GetSyntheticOffersV2QueryValidator()
    {
        RuleFor(x => x.SearchQuery)
            .MaximumLength(200)
            .WithMessage("عبارت جست‌وجو حداکثر ۲۰۰ کاراکتر است.");
        ...
        RuleFor(x => x.SalesModel)
            .Must(GetProductCatalogV2QueryValidator.IsSalesModel)
            .When(x => !string.IsNullOrWhiteSpace(x.SalesModel))
            .WithMessage("مدل فروش معتبر نیست.");
        RuleFor(x => x.OfferKind)
            .Must(IsOfferKind)
            .When(x => !string.IsNullOrWhiteSpace(x.OfferKind))
        ...
        RuleFor(x => x)
            .Must(x => !x.UsageFrom.HasValue || !x.UsageTo.HasValue || x.UsageFrom <= x.UsageTo)
            .WithMessage("تاریخ شروع استفاده باید قبل از تاریخ پایان باشد.");
        RuleFor(x => x.Sort)
            .Must(GetProductCatalogV2QueryValidator.IsSort)
            .WithMessage("مرتب‌سازی معتبر نیست.");
        RuleFor(x => x.PageNumber)
            .GreaterThanOrEqualTo(1)
            .WithMessage("شماره صفحه باید حداقل ۱ باشد.");
        RuleFor(x => x.PageSize)
            .InclusiveBetween(1, 30)
            .WithMessage("اندازه صفحه باید بین ۱ تا ۳۰ باشد.");
    }
