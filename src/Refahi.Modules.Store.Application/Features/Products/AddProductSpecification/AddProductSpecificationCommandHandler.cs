...
public class AddProductSpecificationCommandHandler
    : IRequestHandler<AddProductSpecificationCommand, AddProductSpecificationResponse>
{
    ...
    public async Task<AddProductSpecificationResponse> Handle(
        AddProductSpecificationCommand request,
    ...
    )
    {
        ...
        var addedSpec = product
            .Specifications.Where(s => s.Key == request.Key && s.SortOrder == request.SortOrder)
