...
public sealed class GetProductsQueryValidator : AbstractValidator<GetProductsQuery>
{
    ...
    public GetProductsQueryValidator()
    {
        ...
        RuleFor(x => x.PageSize)
            ...
            .WithMessage("تعداد نتایج هر صفحه باید بین ۱ تا ۳۰ باشد");
        RuleFor(x => x.Sort)
            .Must(x => AllowedSorts.Contains(x))
            .WithMessage("مرتب‌سازی انتخاب‌شده معتبر نیست");
        RuleFor(x => x.SearchQuery)
            .MaximumLength(200)
            .WithMessage("عبارت جستجو نمی‌تواند بیشتر از ۲۰۰ نویسه باشد");
    }
}
