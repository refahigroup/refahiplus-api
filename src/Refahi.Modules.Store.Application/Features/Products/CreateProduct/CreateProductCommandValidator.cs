...
public class CreateProductCommandValidator : AbstractValidator<CreateProductCommand>
{
    ...
    {
        RuleFor(x => x.AgreementProductId).NotEmpty().WithMessage("شناسه قرارداد محصول الزامی است");

        RuleFor(x => x.Title)
            .NotEmpty()
            .WithMessage("عنوان محصول الزامی است")
            .MaximumLength(300)
            .WithMessage("عنوان محصول نمی‌تواند بیشتر از ۳۰۰ کاراکتر باشد");

        RuleFor(x => x.Slug)
            .NotEmpty()
            .WithMessage("اسلاگ الزامی است")
            .MaximumLength(300)
            .WithMessage("اسلاگ نمی‌تواند بیشتر از ۳۰۰ کاراکتر باشد")
            .Matches(@"^[a-z0-9-]+$")
