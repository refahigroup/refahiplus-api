...
public class PlaceStoreOrderCommandHandler : IRequestHandler<PlaceStoreOrderCommand, PlaceStoreOrderResponse>
{
    ...
    public async Task<PlaceStoreOrderResponse> Handle(PlaceStoreOrderCommand request, CancellationToken cancellationToken)
    {
        ...
        var stockUpdates = new List<(Guid ProductId, int Quantity)>();
        ...
        foreach (var cartItem in cart.Items)
        {
            ...
            if (product.SalesModel == SalesModel.StockBased)
            {
                ...
                if (cartItem.VariantId.HasValue)
                {
                    ...
                    var colorPart = !string.IsNullOrWhiteSpace(variant.Color) ? $" {variant.Color}" : string.Empty;
                    var sizePart = !string.IsNullOrWhiteSpace(variant.Size) ? $" سایز {variant.Size}" : string.Empty;
                    itemTitle = $"{product.Title}{colorPart}{sizePart} - {shopForTitle?.Name ?? string.Empty}";
                ...
                stockUpdates.Add((product.Id, cartItem.Quantity));
            }
            else // SessionBased
            {
        ...
        var createOrderCommand = new CreateOrderCommand(
            ...
            Items: orderItems,
            IdempotencyKey: $"store-order-{request.UserId}-{ticks}");
        ...
        var payCommand = new PayOrderCommand(
            ...
            Allocations: request.WalletAllocations
                ...
                .ToList(),
            IdempotencyKey: $"store-pay-{request.UserId}-{ticks}");
