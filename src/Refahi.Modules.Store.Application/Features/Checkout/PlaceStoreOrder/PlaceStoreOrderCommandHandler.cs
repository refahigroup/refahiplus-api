...
public class PlaceStoreOrderCommandHandler
    : IRequestHandler<PlaceStoreOrderCommand, PlaceStoreOrderResponse>
{
    ...
    )
    {
        ...
        var manualResult = await TryPlaceManualOrderAsync(
            cart,
            request,
            orderIdempotencyKey,
            cancellationToken
        ...
        foreach (var cartItem in cart.Items)
        {
            ...
            if (product is null || product.IsDeleted)
                throw new StoreDomainException(
                    $"محصول '{cartItem.ProductId}' یافت نشد یا حذف شده است",
                    "PRODUCT_NOT_FOUND"
                );

            if (!product.IsAvailable)
                throw new StoreDomainException(
                    $"محصول '{product.Title}' در حال حاضر قابل خرید نیست",
            ...
            if (ap?.CategoryId.HasValue == true)
            {
                var category = await _mediator.Send(
                    new GetCategoryByIdQuery(ap.CategoryId.Value),
                    cancellationToken
                );
            ...
            var priceVariantId =
                salesModel == SalesModel.SessionBased && cartItem.SessionId.HasValue
                    ? null
                    : cartItem.VariantId;
            ...
            if (salesModel == SalesModel.StockBased)
            {
                if (cartItem.SessionId.HasValue)
                    throw new StoreDomainException(
                        ...
                        "INVALID_SESSION_FOR_STOCK_PRODUCT"
                    );

                if (cartItem.VariantId.HasValue)
                {
                    var variant =
                        product.Variants.FirstOrDefault(v => v.Id == cartItem.VariantId.Value)
                        ?? throw new StoreDomainException(
                            $"تنوع محصول '{product.Title}' یافت نشد",
                            "VARIANT_NOT_FOUND"
                        );
                    ...
                    itemTitle =
                        $"{product.Title}{(string.IsNullOrEmpty(variantLabel) ? string.Empty : $" - {variantLabel}")} - {shopForTitle?.Name ?? string.Empty}";
                }
                else
                {
                ...
                metadataJson = JsonSerializer.Serialize(
                    new
                    {
                        ...
                        commission_amount_minor = CalculateCommission(
                            authoritativeUnitPrice * cartItem.Quantity,
                            ap.CommissionPercent
                        ),
                        net_amount_minor = authoritativeUnitPrice * cartItem.Quantity
                            - CalculateCommission(
                                authoritativeUnitPrice * cartItem.Quantity,
                                ap.CommissionPercent
                            ),
            ...
            else // SessionBased
            {
                ...
                {
                    var session =
                        product.Sessions.FirstOrDefault(s => s.Id == cartItem.SessionId.Value)
                        ?? throw new StoreDomainException(
                            $"سانس محصول '{product.Title}' یافت نشد",
                            "SESSION_NOT_FOUND"
                        );

                    if (!session.IsAvailable || session.RemainingCapacity < cartItem.Quantity)
                        throw new StoreDomainException(
                            $"ظرفیت کافی برای سانس '{product.Title}' وجود ندارد",
                            "INSUFFICIENT_CAPACITY"
                        );

                    authoritativeUnitPrice += session.PriceAdjustment;

                    var sessionTitlePart = !string.IsNullOrWhiteSpace(session.Title)
                        ? $" {session.Title}"
                        : string.Empty;
                    itemTitle =
                        $"{product.Title}{sessionTitlePart} {session.Date:yyyy-MM-dd} - {shopForTitle?.Name ?? string.Empty}";
        ...
        if (requiresShipping && !request.ShippingAddressId.HasValue)
            throw new StoreDomainException(
                "انتخاب آدرس ارسال الزامی است",
                "SHIPPING_ADDRESS_REQUIRED"
            );

        if (requiresShipping && !request.DeliveryDate.HasValue)
            throw new StoreDomainException(
                "انتخاب تاریخ ارسال الزامی است",
        ...
        if (normalizedShippingAddressId.HasValue)
        {
            ...
            if (addressDto is null)
                throw new StoreDomainException(
                    "آدرس ارسال نامعتبر است یا متعلق به این کاربر نیست",
                    "INVALID_SHIPPING_ADDRESS"
                );

            addressSnapshotJson = JsonSerializer.Serialize(
                new
                {
                    id = addressDto.Id,
        ...
        foreach (var (productId, variantId, quantity) in stockUpdates)
        {
            ...
            for (var attempt = 1; attempt <= 3; attempt++)
            {
                ...
                {
                    product =
                        await _productRepo.GetByIdAsync(productId, cancellationToken)
    ...
    )
    {
        ...
        {
            var product =
                await _productRepo.GetByIdAsync(item.ProductId, ct)
                ?? throw new StoreDomainException("محصول یافت نشد", "PRODUCT_NOT_FOUND");
            var ap =
        ...
        var manualItems = resolved
            .Where(x => x.AgreementProduct.PricingMode == (short)PricingMode.Manual)
            .ToList();
        if (manualItems.Count == 0)
            return null;
        if (resolved.Count != 1 || manualItems.Count != 1)
            throw new StoreDomainException(
                "سفارش حضوری فقط می‌تواند شامل یک آیتم باشد",
        ...
        if (
            ...
            || selected.Item.VariantId.HasValue
            || selected.Item.SessionId.HasValue
            || selected.Item.UsageDate.HasValue
            || selected.Item.UnitPriceMinor <= 0
            ...
            || selected.Product.IsDeleted
        )
            throw new StoreDomainException(
                "ساختار محصول حضوری معتبر نیست",
                "INVALID_MANUAL_PRODUCT"
            );

        var shop =
            await _shopRepo.GetByIdAsync(selected.Item.ShopId, ct)
            ?? throw new StoreDomainException("فروشگاه یافت نشد", "SHOP_NOT_FOUND");
        if (
            shop.Status != ShopStatus.Active
            || shop.ShopType != ShopType.Physical
            || selected.AgreementProduct.SupplierId != shop.SupplierId
        )
            throw new StoreDomainException(
                "فروشگاه حضوری معتبر یا فعال نیست",
                "INVALID_IN_PERSON_SHOP"
            );

        var financial = await _financialPlanner.BuildAsync(
            shop.SupplierId,
            selected.Item.UnitPriceMinor,
            selected.AgreementProduct.CommissionPercent,
            selected.AgreementProduct.VatApplicable,
            ct
        );
        ...
        {
            var category = await _mediator.Send(
                new GetCategoryByIdQuery(selected.AgreementProduct.CategoryId.Value),
                ct
            );
        ...
        var metadata = JsonSerializer.Serialize(
            new
            {
                ...
                pricing_mode = "Manual",
                gross_amount_minor = financial.GrossAmountMinor,
                commission_percent = financial.CommissionPercent,
                commission_amount_minor = financial.CommissionAmountMinor,
                vat_percent = financial.VatPercent,
                vat_amount_minor = financial.VatAmountMinor,
        ...
        var created = await _mediator.Send(
            new CreateOrderCommand(
                ...
                IdempotencyKey: orderIdempotencyKey,
                ReferenceType: "StoreInPerson",
                SourceOwnerId: shop.SupplierId,
