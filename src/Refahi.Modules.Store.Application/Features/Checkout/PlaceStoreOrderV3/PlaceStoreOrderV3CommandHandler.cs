...
{
    public async Task<PlaceStoreOrderV3Response> Handle(
        PlaceStoreOrderV3Command request,
        CancellationToken ct
    )
    {
        var requestFingerprint = CheckoutRequestFingerprint.Create(request);
        var existing = await storeOrders.GetByIdempotencyKeyAsync(
            request.UserId,
        ...
        if (cart.Items.Any(x => !x.OfferId.HasValue))
            throw new StoreDomainException(
                "سبد خرید شامل آیتم قدیمی است؛ سبد v3 را دوباره ایجاد کنید",
        ...
        {
            var current = await offers.ResolveAsync(
                item.ProductId,
                item.ShopId,
                item.VariantId,
                item.SessionId,
            ...
            )
            {
                drift.Add(
                    new OfferChangedDetail(
                        ...
                        current is null
                            ? "پیشنهاد دیگر معتبر نیست"
                            : "پیشنهاد یا قیمت جاری تغییر کرده است"
                    )
                );
                continue;
            }
            ...
            try
            {
                context = await eligibility.ResolveByIdAsync(
                    item.OfferId!.Value,
                    item.Quantity,
                    item.VariantId,
                    item.SessionId,
                ...
                when (ex.ErrorCode.StartsWith("OFFER_", StringComparison.Ordinal)
            ...
            if (context.Product.SalesModel == SalesModel.SessionBased && item.VariantId.HasValue)
            {
                ...
                var usage = StoreVariantCapacityService.NormalizeAndValidateUsageDate(
                    variant,
                    item.UsageDate
                );
            ...
            var category =
                await mediator.Send(new GetCategoryByIdQuery(context.Product.CategoryId), ct)
            ...
            snapshots.Add(
                new StoreOrderItemSnapshot(
                    ...
                    GetVariantTitle(context.Product, item.VariantId),
                    GetSessionTitle(context.Product, item.SessionId),
        ...
        if (drift.Count > 0)
            throw new OfferChangedException(drift);

        var requiresShipping = snapshots.Any(x =>
            x.FulfillmentMethod == FulfillmentMethod.Shipping
        );
        if (
            requiresShipping
            && (!request.ShippingAddressId.HasValue || !request.DeliveryDate.HasValue)
        )
            throw new StoreDomainException(
                "آدرس و تاریخ ارسال الزامی است",
                "SHIPPING_DETAILS_REQUIRED"
            );
        if (requiresShipping)
        {
            var address = await mediator.Send(
        ...
        {
            var concurrent = await storeOrders.GetByIdempotencyKeyAsync(
                request.UserId,
                request.IdempotencyKey,
                ct
            );
    ...
    private async Task<PlaceStoreOrderV3Response> ResumeAsync(
    ...
    )
    {
        ...
        if (storeOrder.OrderId.HasValue)
        {
            var existingOrder =
                await mediator.Send(
                    new GetOrderByIdempotencyKeyQuery(key, storeOrder.UserId, "Store"),
                    ct
                )
                ?? throw new StoreDomainException(
                    "سفارش متصل‌شده یافت نشد",
                    "ATTACHED_ORDER_NOT_FOUND"
                );
            return Map(
                storeOrder,
                existingOrder.Id,
        ...
        var items = storeOrder
            .Items.Select(x => new CreateOrderItemInput(
                BuildTitle(x),
                x.FinalUnitPriceMinor,
                x.Quantity,
                0,
                x.Id,
                x.CategoryCode,
                ["store", "online", $"shop:{x.ShopId:N}"],
                JsonSerializer.Serialize(
                    new
                    {
                        store_order_id = storeOrder.Id,
                        store_order_item_id = x.Id,
                        offer_id = x.OfferId,
                        product_id = x.ProductId,
                        variant_id = x.ProductVariantId,
                        session_id = x.ProductSessionId,
                        agreement_id = x.AgreementId,
                        agreement_category_term_id = x.AgreementCategoryTermId,
                        commission_percent = x.CommissionPercent,
                        commission_amount_minor = x.CommissionAmountMinor,
                        sales_model = x.SalesModel.ToString(),
                        fulfillment_method = x.FulfillmentMethod.ToString(),
        ...
        var requiresShipping = storeOrder.Items.Any(x =>
            x.FulfillmentMethod == FulfillmentMethod.Shipping
        );
    ...
    private static short ResolveDeliveryMethod(
    ...
    )
    {
        ...
        if (
            selections is null
            || !selections.TryGetValue(cartItemId, out var method)
    ...
    private static string BuildTitle(StoreOrderItem x) =>
        string.Join(
            " - ",
            new[] { x.ProductTitle, x.VariantTitle, x.SessionTitle }.Where(v =>
                !string.IsNullOrWhiteSpace(v)
            )
        );

    private static string? GetVariantTitle(Product product, Guid? id) =>
        id.HasValue ? product.Variants.FirstOrDefault(x => x.Id == id)?.SKU : null;

    private static string? GetSessionTitle(Product product, Guid? id) =>
        id.HasValue ? product.Sessions.FirstOrDefault(x => x.Id == id)?.Title : null;

    private static string BuildOrderKey(Guid id) => $"store-order-v3:{id:N}";

    private static PlaceStoreOrderV3Response Map(
        StoreOrder storeOrder,
        Guid orderId,
        string orderNumber,
        long amount
