...
) : IRequestHandler<GetStoreVendorContextsQuery, IReadOnlyList<StoreVendorContextDto>>
{
    public async Task<IReadOnlyList<StoreVendorContextDto>> Handle(
        GetStoreVendorContextsQuery request,
        CancellationToken ct
    )
    {
        var grants = await mediator.Send(
            new GetActiveAuthorizationGrantsQuery(request.UserId, StoreGrantCodec.Issuer),
            ct
        );
        ...
        foreach (var grant in grants)
        {
            if (StoreGrantCodec.TryParse(grant.Value, out var value) && value is not null)
                ...
                logger.LogWarning(
                    "Invalid Store grant ignored. GrantId={GrantId}, UserId={UserId}",
                    grant.Id,
                    grant.UserId
                );
        }

        var vendorIds = parsed
            .Where(x => x.ResourceType == "vendor")
            .Select(x => x.ResourceId)
            .ToHashSet();
        foreach (
            var shopId in parsed
                .Where(x => x.ResourceType == "shop")
                .Select(x => x.ResourceId)
                .Distinct()
        )
        {
        ...
        foreach (var vendorId in vendorIds)
        {
            ...
            if (vendor is null || vendor.Status != 3)
                continue;

            var vendorRoles = parsed
                .Where(x => x.ResourceType == "vendor" && x.ResourceId == vendorId)
                .Select(x => x.Role)
                .Distinct()
                .ToArray();
            ...
            {
                var shopRoles = parsed
                    .Where(x => x.ResourceType == "shop" && x.ResourceId == shop.Id)
                    .Select(x => x.Role)
                    .Concat(vendorRoles)
                    .Distinct()
                    .ToArray();
                ...
                shopDtos.Add(
                    new(
                        ...
                        shopRoles,
                        PermissionsFor(shopRoles, shop.Id, vendorId)
                    )
                );
            }

            if (vendorRoles.Length == 0 && shopDtos.Count == 0)
                continue;
            var name =
                vendor.BrandName
                ?? vendor.CompanyName
                ?? string.Join(
                    ' ',
                    new[] { vendor.FirstName, vendor.LastName }.Where(x =>
            ...
            result.Add(
                new(
                    ...
                    name,
                    vendorRoles,
                    PermissionsFor(vendorRoles, null, vendorId),
                    shopDtos
                )
            );
        }
        ...
        foreach (var role in roles)
        {
            switch (role)
            {
                case StoreAccessRoles.VendorOwner:
                    result.UnionWith(
                        new[]
                        {
                            ...
                            StorePermissions.ViewIncomeWallet,
                ...
                case StoreAccessRoles.ShopSupervisor:
                    ...
                    result.Add(StorePermissions.RedeemVoucher);
                    if (role == StoreAccessRoles.ShopSupervisor)
                        result.Add(StorePermissions.EditShopProfile);
                    break;
    ...
    : IRequestHandler<GetStoreAccessAssignmentsQuery, IReadOnlyList<StoreAccessAssignmentDto>>
{
    public async Task<IReadOnlyList<StoreAccessAssignmentDto>> Handle(
        GetStoreAccessAssignmentsQuery request,
        CancellationToken ct
    )
    {
        var grants = await mediator.Send(
            new GetAuthorizationGrantsByIssuerQuery(StoreGrantCodec.Issuer),
            ct
        );
        var rows = new List<(AuthorizationGrantDto Grant, ParsedStoreGrant Parsed)>();
        foreach (var grant in grants)
        {
            if (!StoreGrantCodec.TryParse(grant.Value, out var parsed) || parsed is null)
                continue;
            var belongs = parsed.ResourceType == "vendor" && parsed.ResourceId == request.VendorId;
            if (parsed.ResourceType == "shop")
                belongs =
                    (await shops.GetByIdAsync(parsed.ResourceId, ct))?.SupplierId
                    == request.VendorId;
            if (belongs)
                rows.Add((grant, parsed));
        }
        var users = await mediator.Send(
            new GetOrderUserSummariesQuery(rows.Select(x => x.Grant.UserId).Distinct().ToArray()),
            ct
        );
        ...
        return rows.Select(x =>
            {
                ...
                var displayName = string.Join(
                    ' ',
                    new[] { user?.FirstName, user?.LastName }.Where(v =>
                        !string.IsNullOrWhiteSpace(v)
                    )
                );
                return new StoreAccessAssignmentDto(
                    x.Grant.Id,
                    x.Grant.UserId,
                    user?.MobileNumber,
                    string.IsNullOrWhiteSpace(displayName) ? null : displayName,
                    request.VendorId,
                    x.Parsed.ResourceType,
                    x.Parsed.ResourceId,
                    x.Parsed.Role,
                    x.Grant.IsActive,
                    x.Grant.CreatedAt
    ...
    : IRequestHandler<GetStoreAccessSummariesQuery, IReadOnlyList<StoreAccessSummaryDto>>
{
    public async Task<IReadOnlyList<StoreAccessSummaryDto>> Handle(
        GetStoreAccessSummariesQuery request,
        CancellationToken ct
    )
    {
...
{
    public async Task<StoreAccessAssignmentDto> Handle(
        CreateStoreAccessAssignmentCommand request,
        CancellationToken ct
    )
    {
        ...
        await mediator.Send(
            new CreateWalletCommand(request.VendorId, WalletTypeCodes.Provider, "IRR"),
            ct
        );
        var currentAssignments = await mediator.Send(
            new GetStoreAccessAssignmentsQuery(request.VendorId),
            ct
        );
        foreach (
            var current in currentAssignments.Where(x =>
            ...
            await mediator.Send(
                new RevokeAuthorizationGrantCommand(current.Id, request.ActorId),
                ct
            );

        var value = StoreGrantCodec.Encode(request.ResourceType, request.ResourceId, request.Role);
        var grant = await mediator.Send(
            new UpsertAuthorizationGrantCommand(
                request.UserId,
                StoreGrantCodec.Issuer,
                value,
                StoreGrantCodec.EmittedRole,
                request.ActorId
            ),
            ct
        );
        return new(
            ...
            null,
            request.VendorId,
            request.ResourceType.ToLowerInvariant(),
            request.ResourceId,
            request.Role,
            true,
            grant.CreatedAt
    ...
    internal static async Task ValidateResource(
        Guid vendorId,
        string resourceType,
        Guid resourceId,
        string role,
    ...
    )
    {
        ...
        var vendor = await mediator.Send(new GetSupplierByIdQuery(vendorId), ct);
        if (vendor is null || vendor.Status != 3)
            throw new InvalidOperationException("Vendor تاییدشده یافت نشد");
        if (
            resourceType.Equals("vendor", StringComparison.OrdinalIgnoreCase)
            && resourceId != vendorId
        )
            throw new InvalidOperationException("شناسه منبع Vendor نامعتبر است");
        if (
            resourceType.Equals("shop", StringComparison.OrdinalIgnoreCase)
...
{
    public async Task<StoreAccessAssignmentDto?> Handle(
        UpdateStoreAccessAssignmentCommand request,
        CancellationToken ct
    )
    {
        ...
        await CreateStoreAccessAssignmentHandler.ValidateResource(
            current.VendorId,
    ...
    : IRequestHandler<RevokeStoreAccessAssignmentCommand, bool>
{
    ...
    {
        var assignment = (
            await mediator.Send(new GetStoreAccessAssignmentsQuery(request.VendorId), ct)
        ).SingleOrDefault(x => x.Id == request.AssignmentId);
        return assignment is not null
            && await mediator.Send(
                new RevokeAuthorizationGrantCommand(request.AssignmentId, request.ActorId),
                ct
            );
    }
}
