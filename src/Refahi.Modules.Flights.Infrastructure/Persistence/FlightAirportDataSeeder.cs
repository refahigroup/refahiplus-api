...
public sealed class FlightAirportDataSeeder
{
    ...
    public FlightAirportDataSeeder(
        FlightsDbContext dbContext,
        ILogger<FlightAirportDataSeeder> logger
    )
    {
    ...
    public async Task SeedAsync(CancellationToken cancellationToken = default)
    {
        ...
        foreach (var record in records)
        {
            ...
            var searchText = FlightAirportSearchNormalizer.Normalize(
                string.Join(
                    ' ',
                    record.IataCode,
                    record.IcaoCode,
        ...
        foreach (
            var airport in existing.Values.Where(airport =>
                !importedCodes.Contains(airport.IataCode)
            )
        )
            airport.Deactivate();
    ...
    private static async Task<IReadOnlyList<AirportSeedRecord>> ReadRecordsAsync(
    ...
    )
    {
        ...
        if (resourceName is null)
            throw new InvalidOperationException(
                $"Embedded airport data '{ResourceSuffix}' was not found."
            );

        await using var resource =
            assembly.GetManifestResourceStream(resourceName)
            ?? throw new InvalidOperationException(
                $"Embedded airport data '{resourceName}' could not be opened."
            );
        await using var gzip = new GZipStream(resource, CompressionMode.Decompress);
        return await JsonSerializer.DeserializeAsync<List<AirportSeedRecord>>(
                gzip,
                JsonOptions,
                cancellationToken
            ) ?? [];
