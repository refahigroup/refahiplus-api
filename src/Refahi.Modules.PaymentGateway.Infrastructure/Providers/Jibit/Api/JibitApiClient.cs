...
public class JibitApiClient
{
    ...
    private static readonly TimeSpan AccessTokenCacheDuration = TimeSpan
        .FromHours(23)
        .Add(TimeSpan.FromMinutes(55));
    private static readonly TimeSpan RefreshTokenCacheDuration = TimeSpan
        .FromHours(47)
        .Add(TimeSpan.FromMinutes(55));
    ...
    /// </summary>
    public async Task<JibitCreatePurchaseResponse> CreatePurchaseAsync(
        JibitCreatePurchaseRequest request,
    ...
    /// </summary>
    public async Task<JibitVerifyResponse> VerifyPurchaseAsync(
        string purchaseId,
        CancellationToken ct
    )
    {
    ...
    private async Task EnsureTokenAsync(CancellationToken ct)
    {
        ...
        if (
            _cache.TryGetValue(RefreshTokenCacheKey, out string? cachedRefreshToken)
        ...
        )
        {
            try
            {
                ...
                {
                    var tokenResult =
                        await refreshResponse.Content.ReadFromJsonAsync<JibitTokenResponse>(ct);
    ...
    private async Task GenerateNewTokenAsync(CancellationToken ct)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            _logger.LogError(
                "Jibit: Token generation failed. Status={Status} Body={Body}",
                response.StatusCode,
                body
            );
            throw new InvalidOperationException(
                $"جیبیت: خطا در دریافت توکن. Status={response.StatusCode}"
            );
        }

        var tokenResult =
            await response.Content.ReadFromJsonAsync<JibitTokenResponse>(ct)
        ...
        where TResponse : class, new()
    {
        ...
        if (response.StatusCode == HttpStatusCode.Unauthorized && !isRetry)
        {
            ...
            await GenerateNewTokenAsync(ct);
            return await SendWithBearerAsync<TRequest, TResponse>(
                method,
                path,
                body,
                isRetry: true,
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            _logger.LogError(
                "Jibit: Request to {Path} failed. Status={Status} Body={Body}",
                path,
                response.StatusCode,
                bodyText
            );
            throw new HttpRequestException(
                $"جیبیت: درخواست به {path} ناموفق بود. Status={response.StatusCode}"
            );
        }

        return await response.Content.ReadFromJsonAsync<TResponse>(ct) ?? new TResponse();
    }
        ...
        where TResponse : class, new()
    {
        ...
        {
            _logger.LogWarning(
                "Jibit: Received 401 on GET {Path}. Forcing token regeneration and retrying.",
                path
            );
            _cache.Remove(AccessTokenCacheKey);
            await GenerateNewTokenAsync(ct);
        ...
        if (!response.IsSuccessStatusCode)
        {
            var bodyText = await response.Content.ReadAsStringAsync(ct);
            _logger.LogError(
                "Jibit: GET {Path} failed. Status={Status} Body={Body}",
                path,
                response.StatusCode,
                bodyText
            );
            throw new HttpRequestException(
                $"جیبیت: درخواست GET به {path} ناموفق بود. Status={response.StatusCode}"
            );
        }
