...
public class PayOrderCommandValidator : AbstractValidator<PayOrderCommand>
{
    public PayOrderCommandValidator()
    {
        ...
        RuleFor(x => x.Allocations)
            .NotEmpty()
            .WithMessage("حداقل یک کیف‌پول برای پرداخت الزامی است");

        RuleForEach(x => x.Allocations)
            .ChildRules(alloc =>
            {
                alloc.RuleFor(a => a.WalletId).NotEmpty().WithMessage("شناسه کیف‌پول الزامی است");

                alloc
                    .RuleFor(a => a.AmountMinor)
                    .GreaterThan(0)
                    .WithMessage("مبلغ پرداختی باید بزرگ‌تر از صفر باشد");
            });
    }
}
