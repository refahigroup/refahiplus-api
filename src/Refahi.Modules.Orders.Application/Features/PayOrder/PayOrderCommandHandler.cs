...
public class PayOrderCommandHandler : IRequestHandler<PayOrderCommand, PayOrderResponse>
{
    ...
    {
        await using var heldLock = await _mutationLock.AcquireAsync(
            request.OrderId,
            cancellationToken
        );
        var order =
            await _orderRepository.GetByIdWithItemsAsync(request.OrderId, cancellationToken)
        ...
        else
        {
            ...
            var categoryCodesForIntent = order
                .Items.Select(i => i.CategoryCode)
                .Where(c => !string.IsNullOrWhiteSpace(c))
                .Distinct()
                .ToList();

            var intentResult = await _mediator.Send(
                new CreatePaymentIntentCommand(
                    ...
                    DestinationWalletId: order.PaymentPostings.Count > 0
                        ? null
                        : request.DestinationWalletId,
                    Postings: order
                        .PaymentPostings.OrderBy(x => x.SortOrder)
    ...
    {
        if (!order.PayableUntil.HasValue || order.PayableUntil.Value > DateTimeOffset.UtcNow)
            return;
