...
public sealed class BalanceRebuilder : IBalanceRebuilder
{
    ...
    public async Task<RebuildBalanceResult> RebuildSingleWalletAsync(
        Guid walletId,
    ...
    public async Task<BatchRebuildResult> RebuildBatchAsync(
    ...
    public async Task<DriftReport> DetectDriftAsync(Guid walletId, CancellationToken ct)
    {
        ...
        var computedSnapshot = new BalanceSnapshot(
        ...
        var drift = new DriftInfo(
            HasDrift: current.AvailableMinor != computed.AvailableMinor
                || current.PendingMinor != computed.PendingMinor,
    ...
    )
    {
        ...
        var currency = await conn.QuerySingleOrDefaultAsync<string>(
            new CommandDefinition(
        ...
        var aggregates = await conn.QueryAsync<LedgerAggregateRow>(
            new CommandDefinition(
    ...
    )
    {
        ...
        var row = await conn.QuerySingleAsync<BalanceRow>(
            new CommandDefinition(
                Sql.SelectCurrentBalance,
                new { WalletId = walletId },
                tx,
