...
public sealed class PaymentIntegrityRepairer : IPaymentIntegrityRepairer
{
    ...
    public PaymentIntegrityRepairer(string connectionString) =>
    ...
    )
    {
        ...
        var intent =
            await connection.QuerySingleOrDefaultAsync<IntentRow>(
                new CommandDefinition(
                    @"
select intent_id as IntentId, order_id as OrderId, status as Status, amount_minor as AmountMinor, currency as Currency
from wallets.payment_intents where intent_id=@IntentId for update",
                    new { IntentId = intentId },
                    transaction,
                    cancellationToken: ct
                )
            ) ?? throw new InvalidOperationException("رزرو پرداخت یافت نشد");
        if (intent.OrderId != expectedOrderId)
            throw new InvalidOperationException("رزرو با سفارش مورد انتظار مطابقت ندارد");

        var allocations = await connection.ExecuteScalarAsync<int>(
            new CommandDefinition(
        ...
        var holds = (
            await connection.QueryAsync<HoldRow>(
                new CommandDefinition(
                    @"
...
where external_reference=@Reference and operation_type=2 and entry_type=3",
                    new { Reference = $"intent:{intentId}" },
                    transaction,
                    cancellationToken: ct
                )
            )
...
where related_entry_id = any(@HoldIds) and operation_type=4 and entry_type=4",
                    new { HoldIds = holds.Select(x => x.LedgerEntryId).ToArray() },
                    transaction,
                    cancellationToken: ct
                )
            )
        ...
        if (intent.Status == 3 && releases.Count == 1)
        {
            ...
            return new(
                intentId,
                intent.OrderId,
                holds.SingleOrDefault()?.WalletId,
                holds.SingleOrDefault()?.LedgerEntryId,
                releases[0],
                intent.AmountMinor,
                dryRun,
                "AlreadyRepaired",
        ...
        if (
            ...
            || holds.Count != 1
            || releases.Count != 0
            || holds[0].AmountMinor != intent.AmountMinor
        )
            throw new InvalidOperationException(
                "الگوی ناسازگاری رزرو با Hold یتیم مورد انتظار مطابقت ندارد؛ عملیات متوقف شد"
            );

        var hold = holds[0];
        await connection.ExecuteAsync(
            new CommandDefinition(
                "select pg_advisory_xact_lock(hashtext(@WalletId::text));",
                new { hold.WalletId },
                transaction,
                cancellationToken: ct
            )
        );
        var before = await connection.QuerySingleAsync<BalanceRow>(
            new CommandDefinition(
                @"
select available_minor as AvailableMinor, pending_minor as PendingMinor
from wallets.wallet_balances where wallet_id=@WalletId for update",
                new { hold.WalletId },
                transaction,
                cancellationToken: ct
            )
...
update wallets.payment_intents set status=3,released_at=@Now where intent_id=@IntentId;",
                new
                {
                    ...
                    hold.Currency,
                    Now = now,
                    HoldId = hold.LedgerEntryId,
                    ExternalReference = $"repair:intent:{intentId}",
                    Metadata = System.Text.Json.JsonSerializer.Serialize(
                        new { repair = "orphan-hold", idempotencyKey }
                    ),
                    IntentId = intentId,
                },
                transaction,
        ...
        var computed = await connection.QuerySingleAsync<ComputedRow>(
            new CommandDefinition(
                @"
select
...
from wallets.ledger_entries where wallet_id=@WalletId",
                new { hold.WalletId },
                transaction,
                cancellationToken: ct
            )
...
update wallets.wallet_balances set available_minor=@AvailableMinor,pending_minor=@PendingMinor,
 last_ledger_entry_id=@ReleaseId,updated_at=@Now,version=version+1 where wallet_id=@WalletId",
                new
                {
                    computed.AvailableMinor,
                    computed.PendingMinor,
                    ReleaseId = releaseId,
                    Now = now,
                    hold.WalletId,
                },
