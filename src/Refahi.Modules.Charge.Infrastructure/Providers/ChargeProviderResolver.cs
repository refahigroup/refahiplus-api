...
public sealed class ChargeProviderResolver : IChargeProviderResolver
{
    ...
    public ChargeProviderResolver(IEnumerable<IChargeProvider> providers) => _providers = providers;
    public IChargeProvider Get(string providerName) => _providers.FirstOrDefault(x => x.Name.Equals(providerName, StringComparison.OrdinalIgnoreCase))
