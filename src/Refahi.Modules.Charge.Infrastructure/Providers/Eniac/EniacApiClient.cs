    ...
    private async Task<string> GetTokenAsync(bool force, CancellationToken ct)
    {
        ...
        try
        {
            ...
            _token = data.GetProperty("token").GetString() ??
                throw new InvalidOperationException("توکن تامین‌کننده دریافت نشد");
