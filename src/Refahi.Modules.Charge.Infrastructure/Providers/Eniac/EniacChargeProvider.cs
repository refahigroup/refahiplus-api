...
public sealed class EniacChargeProvider : IChargeProvider
{
    ...
    {
        using var doc = await _api.GetAsync("/api/Operator/GetPackageType", ct);
    ...
    {
        using var doc = await _api.GetAsync("/api/Operator/EnumOperatorChannelTypes", ct);
    ...
    public async Task<ProviderPurchaseResultDto> PurchaseAsync(ProviderPurchaseRequest r, CancellationToken ct)
    {
        ...
        {
            path = r.Operator switch
            {
                ChargeOperator.Irancell => "/api/Merchant/BuyIrancell",
                ChargeOperator.Mci => "/api/Merchant/BuyMCI",
        ...
        using (doc = await _api.PostAsync(path, body, false, ct, "Purchase", r.CallContext))
        {
            ...
            var pins = ReadPins(data);

            return new(
                EniacJson.Bool(root, "success"),
                EniacJson.Int(root, "eniacResultCode"),
                EniacJson.String(root, "operatorResaultCode", "operatorResultCode"),
                EniacJson.String(root, "message"),
    ...
    public async Task<ProviderTraceResultDto> TraceAsync(ProviderTraceRequest r, CancellationToken ct)
    {
        ...
        return new(
            EniacJson.Bool(root, "success"),
            EniacJson.Int(root, "eniacResultCode"),
            EniacJson.String(data, "operatorResaultCode", "operatorResultCode"),
            EniacJson.String(root, "message", "eniacResultMessage"),
            EniacJson.String(data, "rrn"),
    ...
    {
        using var doc = await _api.GetAsync("/api/Wallet/GetBallance", ct);

        var data = EniacJson.Data(doc.RootElement);

        return new(
            EniacJson.Long(data, "totalBalance"),
            EniacJson.Long(data, "suspendedBalance"),
            EniacJson.Long(data, "availableBalance"),
    ...
    private async Task<ProviderReportDto> ReportAsync(string path, ProviderReportRequest r, CancellationToken ct)
    {
        ...
        return new(
            data.ValueKind == JsonValueKind.Undefined ? "null" : data.GetRawText(),
            EniacJson.Int(root, "totalCount"),
            EniacJson.Bool(root, "success"),
            EniacJson.Int(root, "eniacResultCode"),
            EniacJson.String(root, "message")
        );
