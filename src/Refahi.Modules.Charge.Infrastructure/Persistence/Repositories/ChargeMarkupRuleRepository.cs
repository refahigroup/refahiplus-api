...
public sealed class ChargeMarkupRuleRepository : IChargeMarkupRuleRepository
{
    ...
    public ChargeMarkupRuleRepository(ChargeDbContext db) => _db = db;
    public Task<ChargeMarkupRule?> GetAsync(Guid id, CancellationToken ct = default) => _db.MarkupRules.FirstOrDefaultAsync(x => x.Id == id, ct);
    public async Task<IReadOnlyList<ChargeMarkupRule>> GetAllAsync(CancellationToken ct = default)
        => await _db.MarkupRules.AsNoTracking().OrderByDescending(x => x.IsActive).ThenByDescending(x => x.EffectiveFrom).ToListAsync(ct);
    ...
    public Task<bool> HasOverlapAsync(Guid? excludingId, ChargeOperator? @operator, ChargeServiceType? serviceType, DateTime from, DateTime? to, CancellationToken ct = default)
        => _db.MarkupRules.AnyAsync(x => x.IsActive && (!excludingId.HasValue || x.Id != excludingId)
            && x.Operator == @operator && x.ServiceType == serviceType
            && (to == null || x.EffectiveFrom < to) && (x.EffectiveTo == null || x.EffectiveTo > from), ct);
    public async Task AddAsync(ChargeMarkupRule rule, CancellationToken ct = default) => await _db.MarkupRules.AddAsync(rule, ct);
