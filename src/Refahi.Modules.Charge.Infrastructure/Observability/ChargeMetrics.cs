...
internal static class ChargeMetrics
{
    ...
    private static readonly Counter<long> ProviderFailures = Meter.CreateCounter<long>(
        "charge.provider.failures"
    );
    private static readonly Counter<long> WorkerHeartbeats = Meter.CreateCounter<long>(
        "charge.worker.heartbeats"
    );
    private static readonly Histogram<int> ReconciliationBatchSize = Meter.CreateHistogram<int>(
        "charge.reconciliation.batch_size"
    );
