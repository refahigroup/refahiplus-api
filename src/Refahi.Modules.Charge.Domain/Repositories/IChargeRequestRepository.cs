...
public interface IChargeRequestRepository
{
    ...
    Task<ChargeRequest?> GetByIdempotencyKeyAsync(
        Guid userId,
        string key,
        CancellationToken ct = default
    );
    Task<IReadOnlyList<ChargeRequest>> GetWorkItemsAsync(
        DateTime nowUtc,
        int take,
        CancellationToken ct = default
    );
    Task<IReadOnlyList<ChargeRequest>> GetExpiredCandidatesAsync(
        DateTime nowUtc,
        int take,
        CancellationToken ct = default
    );
    Task AddAsync(ChargeRequest request, CancellationToken ct = default);
    Task AddFulfillmentAttemptAsync(
