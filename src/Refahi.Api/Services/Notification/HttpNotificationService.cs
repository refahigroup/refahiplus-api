...
public class HttpNotificationService : INotificationService
{
    ...
    )
    {
        try
        {
            ...
            if (
                string.IsNullOrEmpty(response.Destination)
                || string.IsNullOrEmpty(response.DestinationType)
            )
            {
                _logger.LogWarning(
    ...
    private static string MapReceiptType(OtpReceiptType type) =>
        type switch
        {
    ...
    private static OtpReceiptType ParseReceiptType(string type) =>
    ...
    private static string MapOtpType(OtpType type) =>
        type switch
        {
