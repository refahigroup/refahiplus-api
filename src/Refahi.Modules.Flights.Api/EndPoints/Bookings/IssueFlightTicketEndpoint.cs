            ...
            .MapPost(
                ...
                ) =>
                {
                    ...
                    var result = await sender.Send(
                        new IssueFlightTicketCommand(bookingId, userId, callerRole, idempotencyKey),
                        cancellationToken
                    );

                    return Results.Ok(
                        ApiResponseHelper.Success(result, "بلیط پرواز با موفقیت صادر شد.")
                    );
                }
            )
