                ...
                ) =>
                {
                    if (!TryParseDate(departureDate, out var parsedDepartureDate))
                        return Results.BadRequest(
                            ApiResponseHelper.Error("فرمت تاریخ رفت معتبر نیست.")
                        );

                    if (!TryParseDate(returnDate, out var parsedReturnDate))
                        return Results.BadRequest(
                            ApiResponseHelper.Error("فرمت تاریخ برگشت معتبر نیست.")
                        );
                    ...
                    return Results.Ok(
                        ApiResponseHelper.Success(result, "جست‌وجوی پرواز با موفقیت انجام شد.")
                    );
                }
            )
