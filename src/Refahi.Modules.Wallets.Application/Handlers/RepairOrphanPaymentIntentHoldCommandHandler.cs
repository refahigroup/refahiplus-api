...
public sealed class RepairOrphanPaymentIntentHoldCommandValidator
    : AbstractValidator<RepairOrphanPaymentIntentHoldCommand>
{
    public RepairOrphanPaymentIntentHoldCommandValidator()
    {
        RuleFor(x => x.IntentId).NotEmpty().WithMessage("شناسه رزرو الزامی است");
        RuleFor(x => x.ExpectedOrderId)
            .NotEmpty()
            .WithMessage("شناسه سفارش مورد انتظار الزامی است");
    ...
    : IRequestHandler<RepairOrphanPaymentIntentHoldCommand, OrphanHoldRepairResult>
{
    ...
    public RepairOrphanPaymentIntentHoldCommandHandler(IPaymentIntegrityRepairer repairer) =>
        _repairer = repairer;

    public Task<OrphanHoldRepairResult> Handle(
        RepairOrphanPaymentIntentHoldCommand request,
        CancellationToken ct
    ) =>
        _repairer.RepairOrphanHoldAsync(
            request.IntentId,
            request.ExpectedOrderId,
            request.DryRun,
