...
public sealed class CreatePaymentIntentApplicationService
{
    ...
    public CreatePaymentIntentApplicationService(
        IPaymentAtomicWriter atomicWriter,
        IWalletReadRepository walletReadRepo
    )
    {
    ...
    )
    {
        ...
        if (totalAllocationAmount != command.AmountMinor)
            throw new InvalidOperationException(
                $"Sum of allocations ({totalAllocationAmount}) does not match total amount ({command.AmountMinor})."
            );
        ...
        foreach (var alloc in command.Allocations)
        {
            if (alloc.AmountMinor <= 0)
                throw new InvalidOperationException(
                    $"Allocation amount must be positive: {alloc.WalletId}"
                );
        }
        ...
        {
            if (
                posting.WalletId == Guid.Empty
                || posting.AmountMinor <= 0
                || posting.Direction is not 1 and not 2
                || string.IsNullOrWhiteSpace(posting.Purpose)
            )
                throw new InvalidOperationException("طرح ثبت‌های مالی معتبر نیست.");

            var wallet =
                await _walletReadRepo.GetByIdAsync(posting.WalletId, ct)
        ...
        if (postings.Count > 0)
        {
            if (command.DestinationWalletId.HasValue)
                throw new InvalidOperationException(
                    "پرداخت چندمقصدی نمی‌تواند کیف مقصد قدیمی داشته باشد."
                );
        ...
        {
            var destination =
                await _walletReadRepo.GetByIdAsync(command.DestinationWalletId.Value, ct)
        ...
        {
            var walletInfo =
                await _walletReadRepo.GetByIdAsync(alloc.WalletId, ct)
            ...
            if (
                walletInfo.AllowedCategoryCode is not null
                && command.OrderItemCategoryCode?.Count > 0
            )
            {
                ...
                if (!allMatch)
                    throw new WalletOperationNotAllowedException(
                        alloc.WalletId,
                        "این کیف پول سازمانی برای دسته‌بندی کالاهای سفارش مجاز نیست."
                    );
            }
        }

        // 4) Convert to Infrastructure input
        var allocInput = command
            .Allocations.Select(a => new AllocationInput(a.WalletId, a.AmountMinor))
            .ToList();
        ...
        var atomicResult = await _atomicWriter.ExecuteCreateIntentAsync(
            ...
            ct: ct,
            postings: postings
                .Select(x => new PaymentPostingInput(
                    x.WalletId,
                    x.Direction,
                    x.AmountMinor,
                    x.Purpose.Trim()
        ...
        {
            CreateIntentOutcome.Created or CreateIntentOutcome.CreatedCached =>
    ...
    )
    {
        var allocations = atomicResult
            .Allocations.Select(a => new AllocationResponse(a.WalletId, a.AmountMinor))
            .ToList();
