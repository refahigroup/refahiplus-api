...
{
    public async Task<AuthorizationGrantDto> Handle(UpsertAuthorizationGrantCommand request, CancellationToken ct)
    {
        if (await users.GetByIdAsync(request.UserId, ct) is null)
            throw new InvalidOperationException("کاربر یافت نشد");

        var grant = await grants.GetAsync(request.UserId, request.Issuer.Trim(), request.Value.Trim(), ct);
        if (grant is null)
        {
            grant = AuthorizationGrant.Create(
                request.UserId, request.Issuer, request.Value, request.EmittedRole, request.ActorId);
    ...
    internal static AuthorizationGrantDto Map(AuthorizationGrant x) => new(
        x.Id, x.UserId, x.Issuer, x.Value, x.EmittedRole, x.IsActive,
        x.CreatedAt, x.CreatedBy, x.RevokedAt, x.RevokedBy);
}
    ...
    : IRequestHandler<GetActiveAuthorizationGrantsQuery, IReadOnlyList<AuthorizationGrantDto>>
{
    public async Task<IReadOnlyList<AuthorizationGrantDto>> Handle(
        GetActiveAuthorizationGrantsQuery request, CancellationToken ct) =>
...
{
    public async Task<IReadOnlyList<AuthorizationGrantDto>> Handle(
        GetAuthorizationGrantsByIssuerQuery request, CancellationToken ct) =>
