...
public sealed class AgreementCategoryTermDomainTests
{
    ...
    public void Create_update_and_soft_delete_preserve_locked_business_shape()
    {
        ...
        var term = agreement.AddCategoryTerm(
            12,
            SalesChannel.Online | SalesChannel.InPerson,
            12.25m
        );
    ...
    [InlineData(1, 1, -1)]
    [InlineData(1, 1, 101)]
    public void Invalid_business_values_have_persian_domain_errors(
        int categoryId,
        short channels,
        decimal commission
    )
    ...
    public void Approved_agreement_terms_are_immutable()
    {
        ...
        var remove = Assert.Throws<SupplyChainDomainException>(() =>
            agreement.RemoveCategoryTerm(term.Id)
        );
