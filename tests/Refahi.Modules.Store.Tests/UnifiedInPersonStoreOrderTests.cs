...
public sealed class UnifiedInPersonStoreOrderTests
{
    ...
    public void CreateInPerson_PersistsSingleDeclaredGrossSnapshot()
    {
        ...
        var order = StoreOrder.CreateInPerson(
            userId,
            actorId,
            "Vendor",
            shopId,
            ...
            "vendor:key",
            new string('a', 64),
            Snapshot(shopId, supplierId, 1_250_000)
        );
    ...
    public void CreateInPerson_RejectsNonPositiveDeclaredGross(long amount)
    {
        var ex = Assert.Throws<StoreDomainException>(() =>
            StoreOrder.CreateInPerson(
                ...
                "user:key",
                new string('b', 64),
                Snapshot(Guid.NewGuid(), Guid.NewGuid(), amount)
            )
        );
        Assert.Contains(
            ex.ErrorCode,
            new[] { "INVALID_IN_PERSON_ITEM", "INVALID_STORE_ORDER_ITEM" }
        );
    }
    ...
    public void OtpChallengeAndVerification_AreRetrySafe()
    {
        ...
        var order = StoreOrder.CreateInPerson(
            ...
            "vendor:key",
            new string('d', 64),
            Snapshot(shopId, supplierId, 100_000)
        );
    ...
    {
        var properties = typeof(StartUserInPersonOrderCommand)
            .GetProperties()
            .Select(x => x.Name)
            .ToArray();
        ...
        Assert.Equal(
            new[] { "UserId", "ShopId", "ProductId", "AmountMinor", "IdempotencyKey" },
            properties
        );
    }
    ...
    public async Task VendorReadProjection_DistinguishesOnlineFromInPerson()
    {
        ...
        var handler = new GetVendorStoreOrdersByOrderIdsHandler(
            repository,
            new ContextMediator(Context(actor, supplier, shop, StorePermissions.ViewOrders))
        );

        var result = await handler.Handle(
            new(actor, [online.OrderId!.Value, inPerson.OrderId!.Value]),
            default
        );
    ...
    public async Task VendorReadProjection_HidesStoreOrderFromNonOwner()
    {
        ...
        var mediator = new ContextMediator(
            Context(nonOwner, supplier, shop, StorePermissions.ViewOwnOrders)
        );
    ...
    public async Task VendorReadProjection_AllowsCashierOwnInPersonOrder()
    {
        ...
        var handler = new GetVendorStoreOrderByOrderIdHandler(
            new ReadRepository([order]),
    ...
    public async Task UserReadProjection_ReturnsMixedOnlineAndInPersonForOwner()
    {
        ...
        var handler = new GetUserStoreOrdersByOrderIdsHandler(
            new ReadRepository([online, inPerson])
        );

        var result = await handler.Handle(
            new(owner, [online.OrderId!.Value, inPerson.OrderId!.Value]),
            default
        );
    ...
    public async Task UserReadProjection_HidesNonOwnerAndAllowsExplicitAdmin()
    {
        ...
        Assert.NotNull(
            await detail.Handle(new(other, order.OrderId.Value, IsAdmin: true), default)
        );
        Assert.Single(
            await batch.Handle(new(other, [order.OrderId.Value], IsAdmin: true), default)
        );
    }
    ...
    public void UserReadEndpoints_RequireUserOrAdminPolicy()
    {
        ...
        builder.Services.AddSingleton<IMediator>(
            new ContextMediator(
                Context(Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), StorePermissions.ViewOrders)
            )
        );
        ...
        var endpoints = ((IEndpointRouteBuilder)app)
            ...
            .Where(x => x.RoutePattern.RawText?.Contains("store-orders") == true)
            .ToArray();
    ...
    private static StoreOrderItemSnapshot Snapshot(Guid shopId, Guid supplierId, long amount) =>
        new(
            Guid.Empty,
            Guid.NewGuid(),
            null,
    ...
    private static StoreOrder InPersonOrder(
        Guid actor,
        Guid supplier,
        Guid shop,
        Guid? buyer = null
    )
    {
        var order = StoreOrder.CreateInPerson(
            ...
            $"vendor:{actor:N}:key",
            new string('f', 64),
            Snapshot(shop, supplier, 750_000)
        );
    ...
    private static StoreVendorContextDto Context(
        Guid actor,
        Guid supplier,
        Guid shop,
        string permission
    ...
    private sealed class ReadRepository(IReadOnlyList<StoreOrder> values) : IStoreOrderRepository
    {
        public Task<StoreOrder?> GetByIdempotencyKeyAsync(
            ...
            Task.FromResult(
                values.SingleOrDefault(x => x.UserId == userId && x.IdempotencyKey == key)
            );
