...
public sealed class CatalogDomainTests
{
    ...
    public void Offer_rejects_invalid_window_and_price()
    {
        ...
        Assert.Throws<StoreDomainException>(() =>
            Offer.Create(Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), null, null, 0, 0, now, null)
        );
        Assert.Throws<StoreDomainException>(() =>
            Offer.Create(Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), null, null, 100, 101, now, null)
        );
        Assert.Throws<StoreDomainException>(() =>
            Offer.Create(Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), null, null, 100, 0, now, now)
        );
    }
    ...
    public void Offer_rejects_discount_scale_above_persisted_precision()
    {
        ...
        var ex = Assert.Throws<StoreDomainException>(() =>
            Offer.Create(Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), null, null, 10_000, 33.335m, now, null)
        );
    ...
    {
        var now = DateTimeOffset.UtcNow;
        var open = Offer.Create(Guid.NewGuid(), 
            Guid.NewGuid(),
            Guid.NewGuid(),
            null,
            null,
        ...
        var later = Offer.Create(Guid.NewGuid(), 
            open.ProductId,
            open.ShopId,
            null,
            null,
        ...
        var winner = Offer.Create(Guid.NewGuid(), 
            open.ProductId,
            open.ShopId,
            null,
            null,
    ...
    public void Resolver_excludes_future_expired_inactive_and_deleted()
    {
        ...
        var p = Guid.NewGuid();
        var s = Guid.NewGuid();
        var future = Offer.Create(Guid.NewGuid(), p, s, null, null, 100, 0, now.AddHours(3), null);
        future.Activate();
        var expired = Offer.Create(Guid.NewGuid(), p, s, null, null, 100, 0, now.AddHours(-2), now.AddHours(1));
        expired.Activate();
        var inactive = Offer.Create(Guid.NewGuid(), p, s, null, null, 100, 0, now.AddHours(-1), null);
        var deleted = Offer.Create(Guid.NewGuid(), p, s, null, null, 100, 0, now.AddHours(-1), null);
        deleted.Activate();
        deleted.SoftDelete();
