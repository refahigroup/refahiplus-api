...
public sealed class CatalogV3PaginationEligibilityTests
{
    ...
    public async Task Create_product_returns_computed_current_eligible_channels(short availableChannel)
    {
        var repository = new FakeProductRepository([]);
        var mediator = new EligibilityMediator(new HashSet<int> { 7 }, availableChannel);
        var command = new CreateProductV3Command(Guid.NewGuid(), true, Guid.NewGuid(), 7, 0,
            (short)ProductType.Goods, (short)SalesModel.Unlimited,
            (short)FulfillmentMethod.Shipping, "محصول", $"product-{availableChannel}", null);

        var result = await new CreateProductV3Handler(repository, mediator)
    ...
    [InlineData((short)SalesChannel.Online)]
    [InlineData((short)SalesChannel.InPerson)]
    public async Task Activation_uses_any_current_valid_channel_without_client_guess(short availableChannel)
    {
        var product = Product.CreateCatalogProduct(Guid.NewGuid(), 7, ProductType.Goods,
            SalesModel.Unlimited, FulfillmentMethod.Shipping, "محصول", $"activation-{availableChannel}");
        product.Suspend();
        var repository = new FakeProductRepository([product]);
        var mediator = new EligibilityMediator(new HashSet<int> { 7 }, availableChannel);
        var deliberatelyWrongClientChannel = availableChannel == (short)SalesChannel.Online
            ? (short)SalesChannel.InPerson : (short)SalesChannel.Online;

        var result = await new SetProductV3ActivationHandler(repository, mediator).Handle(
            new SetProductV3ActivationCommand(Guid.NewGuid(), true, product.Id,
                deliberatelyWrongClientChannel, true), CancellationToken.None);
    ...
    {
        var product = Product.CreateCatalogProduct(Guid.NewGuid(), 7, ProductType.Goods,
            SalesModel.InventoryBased, FulfillmentMethod.Shipping, "محصول", "variant-product");
        ...
        await new UpdateProductVariantCommandHandler(repository, mediator).Handle(
            new UpdateProductVariantCommand(product.Id, created.VariantId, combinations, null,
                4, 12_000, 11_000, "SKU-2"), CancellationToken.None);
    ...
    public async Task Legacy_variant_creation_still_uses_agreement_product_sales_model()
    {
        ...
        await new AddProductVariantCommandHandler(repository, mediator).Handle(
            new AddProductVariantCommand(product.Id,
                [new VariantCombinationInput(attribute.Id, value.Id)], null, 3, 1000, 900, null),
            CancellationToken.None);
    ...
    public void V3_variant_and_session_contracts_do_not_accept_or_expose_parallel_prices()
    {
        ...
        {
            typeof(ProductVariantStructuralV3Request), typeof(CreateProductVariantV3Command),
            typeof(UpdateProductVariantV3Command), typeof(ProductVariantV3Dto),
            typeof(ProductSessionCreateV3Request), typeof(ProductSessionUpdateV3Request),
            typeof(CreateProductSessionV3Command), typeof(UpdateProductSessionV3Command),
            typeof(ProductSessionV3Dto), typeof(AddProductImageCommand),
            typeof(AddProductSpecificationCommand)
        };

        var forbidden = contractTypes.SelectMany(type => type.GetProperties()
    ...
    {
        var product = Product.CreateCatalogProduct(Guid.NewGuid(), 12, ProductType.Goods,
            SalesModel.InventoryBased, FulfillmentMethod.Shipping, "محصول", "structural-variant");
        var attribute = product.AddVariantAttribute("رنگ");
        ...
        var offer = Offer.Create(product.Id, Guid.NewGuid(), null, null, 75_000, 12.50m,
            DateTimeOffset.UtcNow.AddMinutes(-1), null);
        var offerSnapshot = (offer.OriginalPriceMinor, offer.DiscountPercent, offer.FinalPriceMinor,
            offer.Version);

        var created = await handlers.Handle(new CreateProductVariantV3Command(actorId, false,
            product.Id, [new VariantCombinationV3Input(attribute.Id, value.Id)], null, 4, "V3-1"),
            CancellationToken.None);
        ...
        var updated = await handlers.Handle(new UpdateProductVariantV3Command(actorId, false,
            product.Id, stored.Id, [new VariantCombinationV3Input(attribute.Id, value.Id)], null,
            7, "V3-2"), CancellationToken.None);
        Assert.Equal(7, updated.StockCount);
        Assert.Equal(1, stored.PriceMinor);
        Assert.Equal(1, stored.DiscountedPriceMinor);
        Assert.Equal(offerSnapshot, (offer.OriginalPriceMinor, offer.DiscountPercent,
            offer.FinalPriceMinor, offer.Version));
        Assert.Equal(StorePermissions.ManageCatalog, authorization.LastPermission);

        await handlers.Handle(new DeleteProductVariantV3Command(actorId, false, product.Id,
            stored.Id), CancellationToken.None);
        Assert.Empty(product.Variants);

        var deniedHandlers = new ProductV3SubresourceHandlers(repository,
            new FilterMediator(Guid.NewGuid()));
        var denied = await Assert.ThrowsAsync<Refahi.Modules.Store.Domain.Exceptions.StoreDomainException>(() =>
            deniedHandlers.Handle(new CreateProductVariantV3Command(actorId, false, product.Id,
                [new VariantCombinationV3Input(attribute.Id, value.Id)], null, 1, null),
                CancellationToken.None));
    ...
    {
        var product = Product.CreateCatalogProduct(Guid.NewGuid(), 13, ProductType.Service,
            SalesModel.SessionBased, FulfillmentMethod.Voucher, "خدمت", "structural-session");
        var handlers = new ProductV3SubresourceHandlers(new FakeProductRepository([product]),
            new EligibilityMediator(new HashSet<int>()));

        var created = await handlers.Handle(new CreateProductSessionV3Command(Guid.NewGuid(), true,
            product.Id, "2026-08-12", "09:00", "10:00", 8, "صبح"), CancellationToken.None);
        var stored = Assert.Single(product.Sessions);
        Assert.Equal(0, stored.PriceAdjustment);

        var updated = await handlers.Handle(new UpdateProductSessionV3Command(Guid.NewGuid(), true,
            product.Id, created.Id, 10, "صبح جدید", false), CancellationToken.None);
    ...
    {
        var product = Product.CreateCatalogProduct(Guid.NewGuid(), 9, ProductType.Service,
            SalesModel.SessionBased, FulfillmentMethod.Voucher, "خدمت", "session-product");
        var repository = new FakeProductRepository([product]);
        var mediator = new EligibilityMediator(new HashSet<int>());

        await new AddProductImageCommandHandler(repository).Handle(
            new AddProductImageCommand(product.Id, "/media/p.png", true, 0), CancellationToken.None);
        await new AddProductSpecificationCommandHandler(repository).Handle(
            new AddProductSpecificationCommand(product.Id, "مدت", "یک ساعت", 0), CancellationToken.None);
        ...
        var detail = await new AdminGetProductQueryHandler(repository, new StubShopProductRepository(),
            new StubReviewRepository(), mediator, new IdentityPathService()).Handle(
            new AdminGetProductQuery(product.Id), CancellationToken.None);
        ...
        var productDto = new ProductV3Dto(product.Id, product.SupplierId, product.CategoryId,
            (short)product.ProductType, (short)product.SalesModel, (short)product.FulfillmentMethod,
            product.Title, product.Slug, product.Description, product.IsAvailable, product.IsDeleted,
            product.CreatedAt, product.UpdatedAt, product.Version) { EligibleSalesChannels = 3 };
        var vendorDetail = await new GetProductV3ManagementDetailHandler(
            new DetailCompositionMediator(productDto, detail)).Handle(
            new GetProductV3ManagementDetailQuery(product.Id), CancellationToken.None);
        Assert.NotNull(vendorDetail);
        Assert.Equal("Offer", vendorDetail.PricingAuthority);
        Assert.Equal($"/api/store/v3/vendor/products/{product.Id}", vendorDetail.SubresourceBasePath);
        ...
        await new RemoveProductImageCommandHandler(repository).Handle(
            new RemoveProductImageCommand(product.Id, product.Images.Single().Id), CancellationToken.None);
        ...
        string method, string path)
    {
        ...
        var product = Product.CreateCatalogProduct(supplierId, 1, ProductType.Goods,
            SalesModel.InventoryBased, FulfillmentMethod.Shipping, "محصول", "owned");
        var repository = new FakeProductRepository([product]);
        var mediator = new FilterMediator(supplierId);
        var filter = new StoreProviderOwnershipFilter(new StubShopRepository(), repository,
            new StubProductSessionRepository(), mediator);
        ...
        var result = await filter.InvokeAsync(invocation, _ => ValueTask.FromResult<object?>(marker));
    ...
    {
        var userId = Guid.NewGuid();
        var v3 = Product.CreateCatalogProduct(Guid.NewGuid(), 1, ProductType.Goods,
            SalesModel.Unlimited, FulfillmentMethod.Shipping, "جدید", "new-owned");
        var deniedMediator = new FilterMediator(Guid.NewGuid());
        var deniedFilter = new StoreProviderOwnershipFilter(new StubShopRepository(),
            new FakeProductRepository([v3]), new StubProductSessionRepository(), deniedMediator);
        var deniedHttp = ProviderHttp(userId, "POST", "/api/store/provider/products/item/images", v3.Id);
        ...
        var legacyFilter = new StoreProviderOwnershipFilter(new StubShopRepository(),
            new FakeProductRepository([legacy]), new StubProductSessionRepository(), legacyMediator);
        var legacyHttp = ProviderHttp(userId, "POST", "/api/store/provider/products/item/variants", legacy.Id);
        var marker = new object();
    ...
    public void Vendor_capabilities_distinguish_supplier_product_scope_from_shop_offer_scope()
    {
        ...
        var supplierUser = new StoreVendorContextDto(vendorId, "تامین‌کننده", [],
            [StorePermissions.ManageCatalog], []);
        var shopSupervisor = new StoreVendorContextDto(vendorId, "تامین‌کننده", [], [],
            [new VendorShopAccessDto(shopId, "شعبه", "Active", "InPerson", null,
                [StoreAccessRoles.ShopSupervisor], [StorePermissions.ManageCatalog])]);
    ...
    public async Task Shop_supervisor_is_denied_product_scope_but_allowed_offer_for_assigned_shop()
    {
        ...
        var vendorId = Guid.NewGuid();
        var shopId = Guid.NewGuid();
        var context = new StoreVendorContextDto(vendorId, "تامین‌کننده", [], [],
            [new VendorShopAccessDto(shopId, "شعبه", "Active", "InPerson", null,
                [StoreAccessRoles.ShopSupervisor], [StorePermissions.ManageCatalog])]);
        var handler = new AuthorizeStoreResourceHandler(new VendorContextMediator(userId, context));

        Assert.False(await handler.Handle(new AuthorizeStoreResourceQuery(
            userId, vendorId, null, StorePermissions.ManageCatalog), CancellationToken.None));
        Assert.True(await handler.Handle(new AuthorizeStoreResourceQuery(
            userId, vendorId, shopId, StorePermissions.ManageCatalog), CancellationToken.None));
        Assert.False(await handler.Handle(new AuthorizeStoreResourceQuery(
            userId, vendorId, Guid.NewGuid(), StorePermissions.ManageCatalog), CancellationToken.None));
    }
    ...
    public async Task Public_v3_catalog_scopes_module_shop_and_eligibility_before_deduplicated_pagination()
    {
        ...
        var shopA = Guid.NewGuid(); var shopB = Guid.NewGuid();
        var productA = Guid.NewGuid(); var productB = Guid.NewGuid(); var productC = Guid.NewGuid();
        var now = DateTimeOffset.UtcNow;
        var candidates = new List<PublicCatalogOfferCandidate>
        {
            PublicCandidate(productA, supplierA, shopA, "shop-a", "کفش یک", 7, 20_000, now.AddDays(1)),
            PublicCandidate(productA, supplierA, shopB, "shop-b", "کفش یک", 7, 18_000, now.AddDays(1)),
            PublicCandidate(productB, supplierA, shopA, "shop-a", "کفش دو", 7, 30_000, now.AddDays(1)),
            PublicCandidate(productC, supplierB, shopA, "shop-a", "کفش سه", 7, 10_000, now.AddDays(1)),
            PublicCandidate(Guid.NewGuid(), supplierA, shopA, "shop-a", "منقضی", 7, 1_000, now.AddMinutes(-1)),
            PublicCandidate(Guid.NewGuid(), supplierA, shopA, "shop-a", "دسته دیگر", 8, 2_000, now.AddDays(1))
        };
        ...
        var handler = new GetPublicProductCatalogV3Handler(catalog,
            new FakeStoreModuleRepository(StoreModule.Create("فروشگاه", "store", categoryId: 7)), eligibility);

        var first = await handler.Handle(new GetPublicProductCatalogV3Query("store", "کفش", 7,
            null, null, null, null, null, "price-asc", 1, 1), CancellationToken.None);
        var second = await handler.Handle(new GetPublicProductCatalogV3Query("store", "کفش", 7,
            null, null, null, null, null, "price-asc", 2, 1), CancellationToken.None);
        var shopScoped = await handler.Handle(new GetPublicProductCatalogV3Query("store", null, null,
            null, "shop-b", null, null, null, "newest", 1, 30), CancellationToken.None);
        var priceAndModelFiltered = await handler.Handle(new GetPublicProductCatalogV3Query("store",
            null, null, shopA, null, "StockBased", 25_000, 35_000, "price-desc", 1, 30),
            CancellationToken.None);

        Assert.NotNull(first); Assert.NotNull(second); Assert.NotNull(shopScoped);
    ...
    {
        var supplier = Guid.NewGuid(); var shop = Guid.NewGuid();
        var product = Product.CreateCatalogProduct(supplier, 7, ProductType.Service,
            SalesModel.SessionBased, FulfillmentMethod.Voucher, "خدمت", "service-one");
        var repository = new FakeProductRepository([product]);
        await new AddProductImageCommandHandler(repository).Handle(
            new AddProductImageCommand(product.Id, "/service.png", true, 0), CancellationToken.None);
        await new AddProductSpecificationCommandHandler(repository).Handle(
            new AddProductSpecificationCommand(product.Id, "مدت", "یک ساعت", 0), CancellationToken.None);
        var structural = new ProductV3SubresourceHandlers(repository,
            new EligibilityMediator(new HashSet<int>()));
        var session = await structural.Handle(new CreateProductSessionV3Command(Guid.NewGuid(), true,
            product.Id, "2026-08-15", "10:00", "11:00", 5, "صبح"), CancellationToken.None);
        var offerId = Guid.NewGuid();
        var catalog = new FakePublicCatalogRepository([
            PublicCandidate(product.Id, supplier, shop, "main-shop", product.Title, 7, 42_000,
                DateTimeOffset.UtcNow.AddDays(1), productSlug: product.Slug, sessionId: session.Id,
                offerId: offerId)]);
        var handler = new GetPublicProductDetailV3Handler(catalog,
            new FakeStoreModuleRepository(StoreModule.Create("فروشگاه", "store", categoryId: 7)),
            repository, new PublicEligibilityMediator(new HashSet<Guid> { supplier }));

        var detail = await handler.Handle(new GetPublicProductDetailV3Query("store", product.Slug,
            ShopSlug: "main-shop", OfferId: offerId, SessionId: session.Id), CancellationToken.None);
        var wrongShop = await handler.Handle(new GetPublicProductDetailV3Query("store", product.Slug,
            ShopSlug: "other-shop"), CancellationToken.None);

        Assert.NotNull(detail); Assert.Null(wrongShop);
        Assert.Equal("Offer", detail.PricingAuthority);
        Assert.Equal(offerId, detail.SelectedOfferId);
        Assert.Single(detail.Images); Assert.Single(detail.Specifications); Assert.Single(detail.Sessions);
        ...
        Assert.DoesNotContain("Synthetic", detail.GetType().Name, StringComparison.OrdinalIgnoreCase);
        Assert.Empty(typeof(ProductSessionV3Dto).GetProperties().Where(x =>
            x.Name.Contains("Price", StringComparison.OrdinalIgnoreCase)));
        Assert.Empty(typeof(ProductVariantV3Dto).GetProperties().Where(x =>
            x.Name.Contains("Price", StringComparison.OrdinalIgnoreCase)));
    }

    private static PublicCatalogOfferCandidate PublicCandidate(Guid productId, Guid supplierId,
        Guid shopId, string shopSlug, string title, int categoryId, long price,
        DateTimeOffset? endDateUtc, string? productSlug = null, Guid? variantId = null,
        Guid? sessionId = null, Guid? offerId = null) => new(offerId ?? Guid.NewGuid(), productId,
        shopId, supplierId, categoryId, title, productSlug ?? $"p-{productId:N}", null,
        ProductType.Goods, sessionId.HasValue ? SalesModel.SessionBased : SalesModel.InventoryBased,
        FulfillmentMethod.Shipping, DateTimeOffset.UtcNow, "/main.png", "فروشگاه", shopSlug,
        price, 0, price, variantId, sessionId, DateTimeOffset.UtcNow.AddDays(-1), endDateUtc);

    private static DefaultHttpContext ProviderHttp(Guid userId, string method, string path, Guid productId)
    {
    ...
    public async Task Public_products_filter_before_pagination_and_return_exact_total()
    {
        ...
        var products = Enumerable.Range(1, 5)
            .Select(categoryId => Product.CreateCatalogProduct(supplierId, categoryId,
                ProductType.Goods, SalesModel.Unlimited, FulfillmentMethod.Shipping,
                $"محصول {categoryId}", $"product-{categoryId}"))
            .ToList();
        var repository = new FakeProductRepository(products);
        var mediator = new EligibilityMediator(new HashSet<int> { 2, 4, 5 });

        using var cts = new CancellationTokenSource();
        var result = await new ListProductsV3Handler(repository, mediator)
            .Handle(new ListProductsV3Query(null, null, false, 2, 2), cts.Token);
    ...
    public async Task Public_offers_filter_before_pagination_and_use_batch_resolver()
    {
        var now = DateTimeOffset.UtcNow;
        var supplierId = Guid.NewGuid();
        var offers = Enumerable.Range(1, 5)
            .Select(categoryId => (CategoryId: categoryId, Offer: Offer.Create(
                Guid.NewGuid(), Guid.NewGuid(), null, null, 10_000, 10, now.AddDays(-1), null)))
            .ToList();
        var repository = new FakeOfferRepository(offers, supplierId);
        var mediator = new EligibilityMediator(new HashSet<int> { 2, 4, 5 });

        var handler = new OfferQueryHandlers(repository, new FakeProductRepository([]),
            new StubShopRepository(), mediator);
        using var cts = new CancellationTokenSource();
        var result = await handler.Handle(
            new ListOffersQuery(null, null, false, now, 2, 2), cts.Token);
    ...
    private sealed class EligibilityMediator(
        IReadOnlySet<int> eligibleCategories, short eligibleChannels = (short)SalesChannel.Online) : IMediator
    {
        ...
        {
            BatchCalls++;
            return query.Requests.Select(request => new AgreementCategoryTermBatchResult(request,
                eligibleCategories.Contains(request.CategoryId) &&
                (eligibleChannels & request.SalesChannel) != 0
                    ? new ResolvedAgreementCategoryTermDto(Guid.NewGuid(), Guid.NewGuid(), request.SupplierId,
                        request.CategoryId, request.CategoryId, request.SalesChannel, 0,
                        request.AtUtc.AddDays(-1), request.AtUtc.AddDays(1))
                    : null)).ToArray();
        }
        ...
        {
            SingleCalls++;
            throw new InvalidOperationException("Single resolver must not be used by list handlers.");
        }
        ...
        public Task Publish<TNotification>(TNotification notification,
            CancellationToken cancellationToken = default) where TNotification : INotification => Task.CompletedTask;
    }
    ...
    private sealed class PublicEligibilityMediator(IReadOnlySet<Guid> eligibleSuppliers) : IMediator
    {
        ...
        private IReadOnlyList<AgreementCategoryTermBatchResult> Resolve(ResolveAgreementCategoryTermsBatchQuery query)
        {
            BatchCalls++;
            return query.Requests.Select(request => new AgreementCategoryTermBatchResult(request,
                eligibleSuppliers.Contains(request.SupplierId)
                    ? new ResolvedAgreementCategoryTermDto(Guid.NewGuid(), Guid.NewGuid(), request.SupplierId,
                        request.CategoryId, request.CategoryId, request.SalesChannel, 0,
                        request.AtUtc.AddMinutes(-1), request.AtUtc.AddMinutes(1))
                    : null)).ToArray();
        }
        private object Single() { SingleCalls++; throw new InvalidOperationException("N+1 resolver is forbidden."); }
        ...
        : IPublicCatalogRepository
    {
        ...
        public Task<IReadOnlyList<PublicCatalogOfferCandidate>> GetEffectiveCandidatesAsync(
            ...
            string? search, SalesModel? salesModel, DateTimeOffset atUtc, CancellationToken ct = default)
        {
            ModuleCategoryIds.Add(moduleCategoryId);
            var query = candidates.Where(x => x.StartDateUtc <= atUtc &&
                (!x.EndDateUtc.HasValue || atUtc < x.EndDateUtc.Value));
            if (moduleCategoryId.HasValue) query = query.Where(x => x.CategoryId == moduleCategoryId);
            if (categoryId.HasValue) query = query.Where(x => x.CategoryId == categoryId);
            if (shopId.HasValue) query = query.Where(x => x.ShopId == shopId);
            if (!string.IsNullOrWhiteSpace(shopSlug)) query = query.Where(x =>
            ...
            if (salesModel.HasValue) query = query.Where(x => x.SalesModel == salesModel);
    ...
    private sealed class FakeStoreModuleRepository(StoreModule module) : IStoreModuleRepository
    {
        public Task<StoreModule?> GetBySlugAsync(string slug, CancellationToken ct = default) =>
            Task.FromResult<StoreModule?>(module.Slug.Equals(slug, StringComparison.OrdinalIgnoreCase) ? module : null);
        public Task<StoreModule?> GetByIdAsync(int id, CancellationToken ct = default) => Task.FromResult<StoreModule?>(null);
        public Task<List<StoreModule>> GetAllAsync(bool includeInactive = false, CancellationToken ct = default) => Task.FromResult(new List<StoreModule> { module });
        public Task<bool> SlugExistsAsync(string slug, int? excludeId = null, CancellationToken ct = default) => Task.FromResult(module.Slug == slug);
    ...
    private sealed class LegacyAgreementMediator(Guid agreementProductId) : IMediator
    {
        ...
        public Task<TResponse> Send<TResponse>(IRequest<TResponse> request, CancellationToken ct = default)
        {
            if (request is not GetAgreementProductByIdQuery query || query.ProductId != agreementProductId)
                throw new NotSupportedException(request.GetType().FullName);
            AgreementCalls++;
#pragma warning disable CS0618
            object response = new AgreementProductDto(agreementProductId, Guid.NewGuid(), "legacy", null,
                1, null, 1, 1, (short)SalesModel.InventoryBased, 0, false, DateTimeOffset.UtcNow);
#pragma warning restore CS0618
    ...
    private sealed class FilterMediator(Guid allowedSupplierId, Guid? legacyAgreementProductId = null) : IMediator
    {
        ...
        public Task<TResponse> Send<TResponse>(IRequest<TResponse> request, CancellationToken ct = default)
        {
            object? response = request switch
            {
                ...
                GetAgreementProductByIdQuery query when legacyAgreementProductId == query.ProductId => LegacyProduct(query.ProductId),
                GetAgreementByIdQuery query when query.Id == _agreementId => LegacyAgreement(),
                GetAgreementProductByIdQuery => throw new InvalidOperationException("V3 product must not resolve AgreementProduct."),
                _ => throw new NotSupportedException(request.GetType().FullName)
            };
            return Task.FromResult((TResponse)response!);
        }
        private bool Authorize(AuthorizeStoreResourceQuery query)
        {
            ...
            return query.VendorId == allowedSupplierId && query.ShopId is null &&
...
#pragma warning disable CS0618
        ...
        {
            AgreementLookups++;
            return new(id, _agreementId, "legacy", null, 1, null, 1, 1, 1, 0,
                false, DateTimeOffset.UtcNow);
        }
        private AgreementDto LegacyAgreement() => new(_agreementId, "A", 1, "type",
            allowedSupplierId, "supplier", DateTimeOffset.UtcNow.AddDays(-1),
            DateTimeOffset.UtcNow.AddDays(1), 3, "active", null, false,
...
#pragma warning restore CS0618
    ...
    private sealed class DetailCompositionMediator(ProductV3Dto product, ProductDetailDto detail) : IMediator
    {
        public Task<TResponse> Send<TResponse>(IRequest<TResponse> request, CancellationToken ct = default)
        {
            object? response = request switch
    ...
    private sealed class VendorContextMediator(Guid userId, StoreVendorContextDto context) : IMediator
    {
        public Task<TResponse> Send<TResponse>(IRequest<TResponse> request, CancellationToken ct = default)
        {
            object response = request is GetStoreVendorContextsQuery query && query.UserId == userId
                ? new[] { context }
                : throw new NotSupportedException(request.GetType().FullName);
            return Task.FromResult((TResponse)response);
        }
    ...
    private sealed class FakeProductRepository(List<Product> candidates) : IProductRepository
    {
            ...
            IReadOnlyCollection<Guid> eligibleIds, int page, int pageSize, CancellationToken ct = default)
        {
            ...
            return Task.FromResult((eligible.Skip((page - 1) * pageSize).Take(pageSize).ToList(), eligible.Count));
        }
        ...
        public Task<Product?> GetDisplayableBySlugAsync(string slug, IReadOnlyList<Guid> ids, CancellationToken ct = default) => throw new NotSupportedException();
        ...
        public Task<(List<Product> Items, int Total)> SearchAsync(string query, IReadOnlyList<Guid> ids, int page, int pageSize, CancellationToken ct = default) => throw new NotSupportedException();
        ...
        public Task AddProductVariantAsync(Product product, ProductVariant variant, CancellationToken ct = default) => Task.CompletedTask;
    ...
    private sealed class FakeOfferRepository(
        List<(int CategoryId, Offer Offer)> candidates, Guid supplierId) : IOfferRepository
    {
        public IReadOnlyCollection<Guid> LastEligibleIds { get; private set; } = [];
        public CancellationToken LastCancellationToken { get; private set; }
        ...
        {
            LastCancellationToken = ct;
            return Task.FromResult<IReadOnlyList<OfferEligibilityCandidate>>(candidates.Select(x =>
                new OfferEligibilityCandidate(x.Offer.Id, supplierId, x.CategoryId,
                    (short)SalesChannel.Online)).ToArray());
        }
            ...
            IReadOnlyCollection<Guid> eligibleIds, int page, int size, CancellationToken ct = default)
        {
            LastCancellationToken = ct;
            LastEligibleIds = eligibleIds;
            var eligible = candidates.Select(x => x.Offer).Where(x => eligibleIds.Contains(x.Id)).ToList();
            return Task.FromResult(((IReadOnlyList<Offer>)eligible.Skip((page - 1) * size).Take(size).ToList(), eligible.Count));
        }
        public Task<Offer?> GetByIdAsync(Guid id, bool includeDeleted = false, CancellationToken ct = default) => throw new NotSupportedException();
        public Task<bool> HasOpenEndedCoordinateAsync(Guid productId, Guid shopId, Guid? variantId, Guid? sessionId, Guid? excludingId = null, CancellationToken ct = default) => throw new NotSupportedException();
        public Task<(IReadOnlyList<Offer> Items, int Total)> GetPagedAsync(Guid? productId, Guid? shopId, bool includeDeleted, DateTimeOffset? at, int page, int size, CancellationToken ct = default) => throw new NotSupportedException();
        public Task<Offer?> ResolveAsync(Guid productId, Guid shopId, Guid? variantId, Guid? sessionId, DateTimeOffset atUtc, CancellationToken ct = default) => throw new NotSupportedException();
        public Task AddAsync(Offer offer, CancellationToken ct = default) => throw new NotSupportedException();
        public Task UpdateAsync(Offer offer, uint expectedVersion, CancellationToken ct = default) => throw new NotSupportedException();
    }

    private sealed class StubShopRepository : IShopRepository
    {
        public Task<Shop?> GetByIdAsync(Guid id, CancellationToken ct = default) => throw new NotSupportedException();
        ...
        public Task<List<Shop>> GetBySupplierIdAsync(Guid supplierId, CancellationToken ct = default) => throw new NotSupportedException();
    ...
    private sealed class StubProductSessionRepository : IProductSessionRepository
    {
        ...
        public Task<List<ProductSession>> GetByProductIdAsync(Guid productId, CancellationToken ct = default) => throw new NotSupportedException();
        public Task<List<ProductSession>> GetByProductIdAndDateAsync(Guid productId, DateOnly date, CancellationToken ct = default) => throw new NotSupportedException();
        public Task<List<ProductSession>> GetAvailableByProductIdAsync(Guid productId, CancellationToken ct = default) => throw new NotSupportedException();
    ...
    {
        public Task<(IReadOnlyList<ProductOfferingReadModel> Items, int Total)> GetDisplayableProductsAsync(IReadOnlyList<Guid> stockIds, IReadOnlyList<Guid> sessionIds, DateOnly today, string? search, string sort, int page, int pageSize, CancellationToken ct = default) => throw new NotSupportedException();
        ...
        public Task<(List<ShopProduct> Items, int Total)> GetByShopAsync(Guid shopId, bool? active, int page, int pageSize, CancellationToken ct = default) => throw new NotSupportedException();
        public Task<(List<ShopProduct> Items, int Total)> GetByProductAsync(Guid productId, bool? active, int page, int pageSize, CancellationToken ct = default) => throw new InvalidOperationException("V3 detail must not read legacy ShopProduct pricing.");
        ...
        public Task<IReadOnlyList<Guid>> GetActiveShopIdsByAgreementProductIdsAsync(IEnumerable<Guid> ids, CancellationToken ct = default) => throw new NotSupportedException();
        public Task<IReadOnlyList<Guid>> GetDisplayableShopIdsByAgreementProductIdsAsync(IReadOnlyList<Guid> ids, CancellationToken ct = default) => throw new NotSupportedException();
        public Task<(IReadOnlyList<Guid> ProductIds, int Total)> GetDisplayableProductIdsByAgreementProductIdsAsync(IReadOnlyList<Guid> ids, Guid? shopId, int page, int pageSize, CancellationToken ct = default) => throw new NotSupportedException();
        public Task<IReadOnlyDictionary<Guid, ShopProduct>> GetForProductsAsync(IReadOnlyList<Guid> ids, Guid? shopId = null, CancellationToken ct = default) => throw new NotSupportedException();
