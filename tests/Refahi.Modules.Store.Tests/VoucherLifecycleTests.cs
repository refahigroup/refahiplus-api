...
public sealed class VoucherLifecycleTests
{
    ...
    public async Task PaidVoucherItem_IssuesQuantityAndReplayIsIdempotent()
    {
        ...
        Assert.All(
            vouchers.Values,
            voucher => Assert.StartsWith("protected:", voucher.CodeCiphertext)
        );
    }
    ...
    public void VoucherDomain_EnforcesRedeemRevokeAndExpiryTransitions()
    {
        ...
        Assert.Equal(
            ...
            Assert
                .Throws<StoreDomainException>(() => issued.RevokeForRefund("refund", now))
                .ErrorCode
        );
    ...
    public async Task Redeem_IsIdempotentAndDifferentKeyConflicts()
    {
        ...
        var shop = Shop.Create("حضوری", "voucher-shop", ShopType.InPerson, supplier);
        shop.Approve();
        ...
        var handler = new RedeemVoucherHandler(
            repo,
            new FakeShopRepository(shop, shopId),
            new AuthorizationMediator(true),
            TimeProvider.System
        );
        ...
        var ownerDetail = await new GetMyVouchersHandler(repo, protector).Handle(
            new GetMyVoucherQuery(order.UserId, first.VoucherId),
            default
        );
    ...
    public async Task Redeem_RejectsWrongShopAndUnauthorizedUserWithoutCodeLeak()
    {
        ...
        await Issuer(order, repo, protector).Handle(Paid(order), default);
        var code = protector.Plaintexts.Single();
        var wrongShop = Shop.Create("غلط", "wrong-shop", ShopType.InPerson, Guid.NewGuid());
        wrongShop.Approve();
        var wrong = new RedeemVoucherHandler(
            repo,
            new FakeShopRepository(wrongShop, shopId),
            new AuthorizationMediator(true),
            TimeProvider.System
        );
        ...
        var validShop = Shop.Create("صحیح", "valid-shop", ShopType.InPerson, supplier);
        validShop.Approve();
        var denied = new RedeemVoucherHandler(
            repo,
            new FakeShopRepository(validShop, shopId),
        ...
        Assert.Equal(
            ...
            (
                await Assert.ThrowsAsync<VoucherApplicationException>(() =>
                    denied.Handle(new(Guid.NewGuid(), shopId, code, "key-2"), default)
                )
            ).Code
        );
    }
    ...
    public async Task OwnerReadUnprotectsButAuditContractHasNoSecretProperties()
    {
        ...
        var ownerVoucher = Assert.Single(
            await reads.Handle(new GetMyVouchersQuery(order.UserId), cts.Token)
        );
        ...
        Assert.Null(
            await reads.Handle(new GetMyVoucherQuery(Guid.NewGuid(), repo.Values[0].Id), default)
        );
    ...
    {
        var order = StoreOrderWith(FulfillmentMethod.Voucher, 1);
        var issueProtector = new TestProtector();
        var repo = new FakeVoucherRepository();
    ...
    {
        var handler = new GetVendorVoucherRedemptionHistoryHandler(
            new FakeVoucherRepository(),
            new AuthorizationMediator(false)
        );
        var ex = await Assert.ThrowsAsync<VoucherApplicationException>(() =>
            handler.Handle(new(Guid.NewGuid(), Guid.NewGuid(), null), default)
        );
    ...
    {
        var supplier = Guid.NewGuid();
        var shopOne = Guid.NewGuid();
        var shopTwo = Guid.NewGuid();
        var repo = new FakeVoucherRepository();
        var older = await AddRedeemedVoucher(
            repo,
            supplier,
            shopOne,
            "شعبه یک",
        ...
        var newer = await AddRedeemedVoucher(
            repo,
            supplier,
            shopTwo,
            "شعبه دو",
        ...
        var shopScoped = await handler.Handle(
            new(Guid.NewGuid(), supplier, shopOne, 1, 20),
            default
        );
        ...
        var names = typeof(VoucherRedemptionHistoryItemDto)
            .GetProperties()
            .Select(x => x.Name)
            .ToArray();
    ...
    public void RedemptionHistory_ValidatesPaginationBounds()
    {
        ...
        Assert.Contains(
            result.Errors,
            x => x.PropertyName == nameof(GetVendorVoucherRedemptionHistoryQuery.Page)
        );
        Assert.Contains(
            result.Errors,
            x => x.PropertyName == nameof(GetVendorVoucherRedemptionHistoryQuery.PageSize)
        );
    }
    ...
    public async Task RefundRevokesIssuedAndBlocksRedeemedBeforePaymentCoordination()
    {
        ...
        var protector = new TestProtector();
        await Issuer(order, repo, protector).Handle(Paid(order), default);
        var handler = new PrepareStoreOrderRefundHandler(
            new FakeStoreOrderRepository(order),
            repo,
            new FakeVoucherRefundOverrideRepository(),
        ...
        Assert.Equal(
            0,
            (await handler.Handle(new(order.OrderId.Value, "refund"), default)).RevokedCount
        );

        var redeemedOrder = StoreOrderWith(FulfillmentMethod.Voucher, 1);
        var redeemedRepo = new FakeVoucherRepository();
        var redeemedProtector = new TestProtector();
        await Issuer(redeemedOrder, redeemedRepo, redeemedProtector)
            .Handle(Paid(redeemedOrder), default);
        redeemedRepo
            .Values[0]
            .Redeem(Guid.NewGuid(), redeemedOrder.ShopId, "شعبه استفاده", DateTimeOffset.UtcNow);
        var blocked = new PrepareStoreOrderRefundHandler(
            new FakeStoreOrderRepository(redeemedOrder),
            redeemedRepo,
            new FakeVoucherRefundOverrideRepository(),
            new NoopMutationLock(),
            TimeProvider.System
        ...
        Assert.Equal(
            ...
            (
                await Assert.ThrowsAsync<VoucherApplicationException>(() =>
                    blocked.Handle(new(redeemedOrder.OrderId!.Value, "refund"), default)
                )
            ).Code
        );
    }
    ...
    public async Task AuthorizedOverride_PersistsAudit_CompletesOnce_AndKeepsVoucherRedeemed()
    {
        ...
        var command = new OverrideRedeemedVoucherRefundCommand(
            order.OrderId!.Value,
            admin,
            "سرویس قبلاً ارائه شده و بازگشت وجه با تایید مدیر انجام می‌شود",
            "override-key"
        );
        ...
        Assert.DoesNotContain(
            "Code",
            overrides.Values[0].VoucherSnapshotJson,
    ...
    public async Task FailedRefund_KeepsAuditAndRetriesSameDeterministicRefund()
    {
        ...
        var command = new OverrideRedeemedVoucherRefundCommand(
            order.OrderId!.Value,
            Guid.NewGuid(),
            "خطای ارائه سرویس تایید شده و بازگشت وجه استثنایی لازم است",
            "retryable-key"
        );
    ...
    public async Task OverrideIdempotency_RejectsChangedPayload_AndConcurrentAdminReusesAudit()
    {
        ...
        var first = await handler.Handle(
            new(
                order.OrderId!.Value,
                Guid.NewGuid(),
                "دلیل معتبر برای استثنای بازگشت وجه ثبت شد",
                "same-key"
            ),
            default
        );

        var mismatch = await Assert.ThrowsAsync<VoucherApplicationException>(() =>
            handler.Handle(
                new(
                    order.OrderId.Value,
                    Guid.NewGuid(),
                    "دلیل متفاوت و معتبر برای استثنا",
                    "same-key"
                ),
                default
            )
        ...
        var secondAdmin = await handler.Handle(
            new(order.OrderId.Value, Guid.NewGuid(), first.Reason, "another-admin-key"),
            default
        );
    ...
    public async Task ValidOverride_AllowsRefundGuardWithoutUndoingRedeemedVoucher()
    {
        ...
        var snapshot = System.Text.Json.JsonSerializer.Serialize(
            vouchers.Values.Select(x => new VoucherRefundSnapshotItemDto(x.Id, x.Status.ToString()))
        );
        var value = VoucherRefundOverride.Create(
            order.Id,
            order.OrderId!.Value,
            Guid.NewGuid(),
            reason,
    ...
    public void OverrideValidator_RequiresMeaningfulTrimmedReason()
    {
        ...
        var valid = new OverrideRedeemedVoucherRefundCommand(
            Guid.NewGuid(),
            Guid.NewGuid(),
            "  دلیل معتبر برای بازگشت وجه استثنایی  ",
            "key"
        );
    ...
    private static OverrideRedeemedVoucherRefundHandler OverrideHandler(
        StoreOrder order,
        FakeVoucherRepository vouchers,
        FakeVoucherRefundOverrideRepository overrides,
        IMediator mediator
    ) =>
        new(
            new FakeStoreOrderRepository(order),
            ...
            mediator,
            TimeProvider.System,
            NullLogger<OverrideRedeemedVoucherRefundHandler>.Instance
        );
        ...
        Assert.Contains(
            ...
            x =>
                ...
                && x.Properties.Select(p => p.Name)
                    .SequenceEqual([
                        nameof(Voucher.StoreOrderItemId),
                        nameof(Voucher.SequenceNumber),
                    ])
        );
        Assert.Contains(
            voucher.GetIndexes(),
    ...
    public void VoucherEndpoints_HaveRequiredPoliciesAndRateLimitMetadata()
    {
        ...
        new VoucherEndpoints().Map(app.MapGroup("/api/store"));
        var endpoints = ((IEndpointRouteBuilder)app)
            .DataSources.SelectMany(x => x.Endpoints)
            .OfType<RouteEndpoint>()
            .Where(x => x.RoutePattern.RawText?.Contains("vouchers") == true)
            .ToArray();
        Assert.Equal(5, endpoints.Length);
        Assert.Contains(
            endpoints,
            x =>
                x.RoutePattern.RawText!.Contains("redeem")
                && x.Metadata.GetOrderedMetadata<IAuthorizeData>()
                    .Any(a => a.Policy == "VendorOnly")
        ...
        Assert.Contains(
            endpoints,
            x =>
                x.RoutePattern.RawText!.Contains("history")
                && x.Metadata.GetMetadata<EnableRateLimitingAttribute>()?.PolicyName
                    == "VoucherRedeem"
        );
        Assert.Contains(
            endpoints,
            x =>
                x.RoutePattern.RawText!.Contains("admin")
                && x.Metadata.GetOrderedMetadata<IAuthorizeData>().Any(a => a.Policy == "AdminOnly")
        );
        var allEndpoints = ((IEndpointRouteBuilder)app)
            .DataSources.SelectMany(x => x.Endpoints)
            .OfType<RouteEndpoint>()
            .ToArray();
        Assert.Contains(
    ...
    private static IssueVouchersAfterOrderPaidHandler Issuer(
        StoreOrder order,
        FakeVoucherRepository repo,
    ...
    )
    {
        ...
        return new(
            new FakeStoreOrderRepository(order),
            repo,
            new FakeShopRepository(shop, order.ShopId),
            new AuthorizationMediator(true),
            protector,
            TimeProvider.System,
            Microsoft
                ...
                .Logging
                .Abstractions
                .NullLogger<IssueVouchersAfterOrderPaidHandler>
                .Instance
        );
    }
    ...
    private static async Task<Voucher> AddRedeemedVoucher(
        FakeVoucherRepository repo,
        Guid supplierId,
        Guid shopId,
        string shopName,
    ...
    private static OrderPaidIntegrationEvent Paid(StoreOrder order) =>
        new(
            order.OrderId!.Value,
            "SO-1",
            order.UserId,
    ...
    private static StoreOrder StoreOrderWith(
    ...
    {
        var supplierId = supplier ?? Guid.NewGuid();
        var shopId = shop ?? Guid.NewGuid();
        var snapshot = new StoreOrderItemSnapshot(
            Guid.NewGuid(),
            Guid.NewGuid(),
            null,
            null,
    ...
    private sealed class TestProtector : IVoucherCodeProtector
    {
        ...
        {
            Plaintexts.Add(value);
            return "protected:" + value;
        }

        public bool TryUnprotect(string value, out string plaintext)
        {
            plaintext = value.StartsWith("protected:") ? value[10..] : string.Empty;
    ...
    private sealed class FakeVoucherRepository : IVoucherRepository
    {
        ...
        public Task<Voucher?> GetByItemSequenceAsync(
            ...
            Task.FromResult(
                Values.SingleOrDefault(x =>
                    x.StoreOrderItemId == item && x.SequenceNumber == sequence
                )
            );

        public Task<Voucher?> GetByCodeHashAsync(string hash, CancellationToken ct = default) =>
            Task.FromResult(Values.SingleOrDefault(x => x.CodeHash == hash));
        ...
        public Task<IReadOnlyList<Voucher>> GetByUserAsync(Guid id, CancellationToken ct = default)
        {
            ...
            return Task.FromResult<IReadOnlyList<Voucher>>(
                Values.Where(x => x.UserId == id).ToArray()
            );
        }

        public Task<IReadOnlyList<Voucher>> GetAllAsync(CancellationToken ct = default) =>
            Task.FromResult<IReadOnlyList<Voucher>>(Values);

        public Task<IReadOnlyList<Voucher>> GetByStoreOrderAsync(
            Guid id,
            CancellationToken ct = default
        ) =>
            Task.FromResult<IReadOnlyList<Voucher>>(
                Values.Where(x => x.StoreOrderId == id).ToArray()
            );
        ...
        public Task<VoucherRedemption?> GetRedemptionByIdempotencyAsync(
            ...
            Task.FromResult(
                redemptions.SingleOrDefault(x => x.VendorUserId == user && x.IdempotencyKey == key)
            );

        public Task<VoucherRedemptionHistoryPage> GetRedemptionHistoryAsync(
            Guid supplier,
            Guid? shop,
            int page,
            int pageSize,
        ...
        )
        {
            ...
            var query =
                ...
                where
                    redemption.SupplierId == supplier
                    && voucher.SupplierId == supplier
            ...
            return Task.FromResult(
                new VoucherRedemptionHistoryPage(
                    rows.Length,
        ...
        public Task AddAsync(Voucher value, CancellationToken ct = default)
        {
            Values.Add(value);
        ...
        public Task UpdateAsync(Voucher value, CancellationToken ct = default) =>
            Task.CompletedTask;

        public Task UpdateRangeAsync(
            IReadOnlyCollection<Voucher> values,
            CancellationToken ct = default
        ) => Task.CompletedTask;
    }
    ...
    private sealed class FakeStoreOrderRepository(StoreOrder value) : IStoreOrderRepository
    {
        ...
        public Task<StoreOrder?> GetByOrderIdAsync(Guid id, CancellationToken ct = default) =>
            Task.FromResult(value.OrderId == id ? value : null);

        public Task<StoreOrder?> GetByIdAsync(Guid id, CancellationToken ct = default) =>
            Task.FromResult(value.Id == id ? value : null);

        public Task<IReadOnlyList<StoreOrder>> GetByOrderIdsAsync(
            IReadOnlyCollection<Guid> ids,
            CancellationToken ct = default
        ) =>
            Task.FromResult<IReadOnlyList<StoreOrder>>(
                ids.Contains(value.OrderId!.Value) ? [value] : []
            );
    ...
    private sealed class FakeShopRepository(Shop value, Guid expectedId) : IShopRepository
    {
        public Task<Shop?> GetByIdAsync(Guid id, CancellationToken ct = default) =>
            Task.FromResult(id == expectedId ? value : null);

        public Task<Shop?> GetBySlugAsync(string slug, CancellationToken ct = default) =>
            throw new NotSupportedException();

        public Task<Shop?> GetByProviderIdAsync(Guid id, CancellationToken ct = default) =>
            throw new NotSupportedException();

        public Task<List<Shop>> GetBySupplierIdAsync(Guid id, CancellationToken ct = default) =>
            throw new NotSupportedException();
        ...
        public Task<bool> ProviderHasShopAsync(Guid id, CancellationToken ct = default) =>
            throw new NotSupportedException();
    ...
    private sealed class AuthorizationMediator(bool allowed) : IMediator
    {
        ...
        private static SupplierDto Supplier(Guid id) =>
            new(
                id,
                1,
                "حقوقی",
    ...
    {
        public Task<IAsyncDisposable> AcquireAsync(Guid id, CancellationToken ct) =>
    ...
    private sealed class FakeVoucherRefundOverrideRepository : IVoucherRefundOverrideRepository
    {
        ...
        public Task<VoucherRefundOverride?> GetByOrderIdAsync(
            Guid id,
            CancellationToken ct = default
        ) => Task.FromResult(Values.SingleOrDefault(x => x.OrderId == id));

        public Task<VoucherRefundOverride?> GetByIdempotencyKeyAsync(
            string key,
            CancellationToken ct = default
        ) => Task.FromResult(Values.SingleOrDefault(x => x.IdempotencyKey == key));

        public Task<IReadOnlyList<VoucherRefundOverrideAttempt>> GetAttemptsAsync(
            Guid id,
            CancellationToken ct = default
        ) =>
            Task.FromResult<IReadOnlyList<VoucherRefundOverrideAttempt>>(
                Attempts
                    .Where(x => x.VoucherRefundOverrideId == id)
                    .OrderBy(x => x.SequenceNumber)
                    .ToArray()
            );

        public Task AddAsync(VoucherRefundOverride value, CancellationToken ct = default)
        {
            Values.Add(value);
            return Task.CompletedTask;
        }
    ...
    private sealed class RefundOverrideMediator(int failuresBeforeSuccess = 0) : IMediator
    {
        ...
        {
            if (request is not CancelOrderCommand command)
                throw new NotSupportedException();
            CancelCount++;
            IdempotencyKeys.Add(command.IdempotencyKey);
            if (CancelCount <= failuresBeforeSuccess)
                throw new InvalidOperationException("provider detail must not leak");
            return Task.FromResult(
                (TResponse)(object)new CancelOrderResponse(command.OrderId, "Cancelled", "Refunded")
            );
        }
