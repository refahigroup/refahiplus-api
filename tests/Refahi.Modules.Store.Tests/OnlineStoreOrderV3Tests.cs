...
public sealed class OnlineStoreOrderV3Tests
{
    [Fact]
    public void Store_order_is_created_before_order_with_immutable_financial_snapshot()
    {
        ...
        var order = StoreOrder.Create(Guid.NewGuid(), 7, snapshot.ShopId, snapshot.SupplierId,
            "checkout-1", new string('a', 64), [snapshot]);
    ...
    public void Snapshot_does_not_reference_mutable_offer_prices()
    {
        ...
        var offer = Offer.Create(Guid.NewGuid(), Guid.NewGuid(), null, null, 1000, 20, now.AddMinutes(-1), null);
        var snapshot = Snapshot(offer.Id, offer.FinalPriceMinor, offer.OriginalPriceMinor, 1);
        var order = StoreOrder.Create(Guid.NewGuid(), 1, snapshot.ShopId, snapshot.SupplierId,
            "snapshot", new string('b', 64), [snapshot]);
    ...
    public void Store_order_rejects_mixed_shop_items()
    {
        ...
        var ex = Assert.Throws<StoreDomainException>(() => StoreOrder.Create(Guid.NewGuid(), 1,
            first.ShopId, first.SupplierId, "mixed", new string('c', 64), [first, second]));
    ...
    {
        var item = Snapshot();
        var order = StoreOrder.Create(Guid.NewGuid(), 1, item.ShopId, item.SupplierId,
            "lifecycle", new string('d', 64), [item]);
    ...
    public void Ef_model_has_idempotency_order_uniqueness_and_xmin_concurrency()
    {
        ...
        Assert.True(entity.GetIndexes().Single(x => x.Properties.Select(p => p.Name)
            .SequenceEqual([nameof(StoreOrder.UserId), nameof(StoreOrder.IdempotencyKey)])).IsUnique);
        Assert.True(entity.GetIndexes().Single(x => x.Properties.Count == 1 &&
            x.Properties[0].Name == nameof(StoreOrder.OrderId)).IsUnique);
    ...
    {
        var constructors = typeof(Refahi.Modules.Store.Application.Features.Checkout.PlaceStoreOrderV3.PlaceStoreOrderV3CommandHandler)
            .GetConstructors().SelectMany(x => x.GetParameters()).Select(x => x.ParameterType.FullName).ToArray();
        Assert.DoesNotContain(constructors, x => x?.Contains("Wallet", StringComparison.OrdinalIgnoreCase) == true);
    }

    private static StoreOrderItemSnapshot Snapshot(Guid? offerId = null, long final = 800,
        long original = 1000, int quantity = 2) => new(
        Guid.NewGuid(), Guid.NewGuid(), null, null, offerId ?? Guid.NewGuid(), "محصول", null, null,
        11, "store.test", Guid.NewGuid(), Guid.NewGuid(), SalesChannel.Online,
        ProductType.Goods, SalesModel.Unlimited, FulfillmentMethod.Download,
        quantity, original, 20, final, Guid.NewGuid(), Guid.NewGuid(), 10, null);
}
