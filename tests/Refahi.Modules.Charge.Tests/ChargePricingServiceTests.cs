...
public sealed class ChargePricingServiceTests
{
    ...
    public async Task Percentage_and_fixed_markup_are_applied_once()
    {
        ...
        var rule = ChargeMarkupRule.Create(
    ...
    {
        var result = await new ChargePricingService(new RuleRepository(null)).CalculateAsync(
            ChargeOperator.Mci,
        ...
        Assert.Equal(0, result.MarkupAmountMinor);
        Assert.Equal(50_000, result.FinalAmountMinor);
    }
    ...
    private sealed class RuleRepository(ChargeMarkupRule? rule) : IChargeMarkupRuleRepository
    {
        ...
        public Task<IReadOnlyList<ChargeMarkupRule>> GetAllAsync(CancellationToken ct = default) =>
            Task.FromResult<IReadOnlyList<ChargeMarkupRule>>(rule is null ? [] : [rule]);
        ...
        public Task AddAsync(ChargeMarkupRule r, CancellationToken ct = default) =>
            Task.CompletedTask;
