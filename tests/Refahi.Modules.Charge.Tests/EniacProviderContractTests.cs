...
public sealed class EniacProviderContractTests
{
    ...
    public async Task Purchase_transport_failure_is_not_retried()
    {
        ...
        var provider = Provider(handler);
        await Assert.ThrowsAsync<HttpRequestException>(() => provider.PurchaseAsync(new ProviderPurchaseRequest(
    ...
    private static EniacChargeProvider Provider(HttpMessageHandler handler)
    {
