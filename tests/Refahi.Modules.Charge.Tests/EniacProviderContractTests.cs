...
public sealed class EniacProviderContractTests
{
    ...
    public async Task Product_contract_maps_documented_fields()
    {
        var handler = new QueueHandler(
            Json(
                """{"data":{"token":"token","expiration":"2099-01-01T00:00:00+03:30"},"success":true}"""
            ),
            Json(
                """{"data":[{"operatorPackageId":"O23285","titleFa":"بسته روزانه","packageTypeCode":1,"packageTypeName":"روزانه","operatorTypesId":1,"amount":10000,"amountWithTax":11000,"durationDays":1,"isActive":true,"packageCategoryId":1002}],"success":true,"eniacResultCode":0}"""
            )
        );
    ...
    public async Task Purchase_transport_failure_is_not_retried()
    {
        ...
        var exception = await Assert.ThrowsAsync<ChargeProviderException>(() =>
            provider.PurchaseAsync(
    ...
    public async Task Purchase_http_400_with_provider_code_is_definitive()
    {
        var handler = new QueueHandler(
            ...
            Json(
                """{"message":"شما امکان دسترسی به این سرویس را ندارید.","success":false,"eniacResultCode":120}""",
    ...
    public async Task Trace_http_failure_does_not_turn_unknown_purchase_into_definitive_failure()
    {
        var handler = new QueueHandler(
            ...
            Json(
                """{"message":"دسترسی به گزارش تراکنش ممکن نیست","success":false,"eniacResultCode":120}""",
    ...
    public async Task Purchase_server_failure_remains_ambiguous_even_with_provider_code()
    {
        var handler = new QueueHandler(
            ...
            Json(
                """{"message":"خطای داخلی تامین‌کننده","success":false,"eniacResultCode":503}""",
    ...
    private static EniacChargeProvider Provider(
        HttpMessageHandler handler,
        IProviderCallLogRepository? logs = null
    )
    {
        ...
        return new EniacChargeProvider(
            logs is null
