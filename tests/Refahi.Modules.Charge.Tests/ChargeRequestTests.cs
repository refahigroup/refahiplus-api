...
public sealed class ChargeRequestTests
{
    ...
    public void Processing_before_order_payment_is_rejected()
    {
        ...
        Assert.Throws<InvalidOperationException>(() =>
            request.StartProcessing("test", DateTime.UtcNow, TimeSpan.FromMinutes(1))
        );
    }
    ...
    public void Paid_request_can_be_fulfilled_idempotently()
    {
        ...
        request.MarkPaid(orderId, paymentId, now);
    ...
    {
        var now = DateTime.UtcNow;
        var request = Create(now.AddMinutes(-30), now.AddMinutes(-10));
    ...
    public void Provider_invoice_number_respects_eniac_contract()
    {
        ...
        Assert.Equal(
            ChargeRequest.ProviderInvoiceNumberMaxLength,
            first.CustomerInvoiceNumber.Length
        );
    ...
    public void Legacy_invoice_number_is_normalized_before_first_provider_call()
    {
        ...
        typeof(ChargeRequest)
            .GetProperty(nameof(ChargeRequest.CustomerInvoiceNumber))!
        ...
        Assert.Equal(
            ChargeRequest.ProviderInvoiceNumberMaxLength,
            request.CustomerInvoiceNumber.Length
        );
