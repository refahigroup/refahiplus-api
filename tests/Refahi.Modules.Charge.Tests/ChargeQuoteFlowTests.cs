...
public sealed class ChargeQuoteFlowTests
{
    ...
    public async Task Preview_calculates_price_without_creating_request()
    {
        ...
        var quote = await handler.Handle(
            new PreviewChargeRequestCommand(
    ...
    public async Task Changed_price_does_not_persist_request()
    {
        ...
        var exception = await Assert.ThrowsAsync<ChargeQuoteChangedException>(() =>
            handler.Handle(
                new CreateChargeRequestCommand(
                    Guid.NewGuid(),
                    ChargeOperator.Rightel,
                    ChargeServiceType.DirectCharge,
                    "09210000000",
    ...
    private static ChargeRequestQuoteService CreateQuoteService()
    {
        var configuration = new ConfigurationBuilder()
            .AddInMemoryCollection(
                new Dictionary<string, string?> { ["Charge:RequestTtlMinutes"] = "20" }
            )
            .Build();
    ...
    private sealed class FakeChargeProvider : IChargeProvider
    {
        ...
        public Task<IReadOnlyList<ChargeProductDto>> GetProductsAsync(
            ChargeOperator @operator,
            CancellationToken ct
        ) => throw new NotSupportedException();

        public Task<IReadOnlyList<ChargeProductDto>> GetOffersAsync(
            ChargeOperator @operator,
            string mobileNumber,
        ...
        public Task<ChargeEligibilityDto> CheckEligibilityAsync(
            ChargeEligibilityRequest request,
            CancellationToken ct
        ) => throw new NotSupportedException();

        public Task<ChargePostpaidBalanceDto> GetPostpaidBalanceAsync(
            ChargeOperator @operator,
            string mobileNumber,
            CancellationToken ct
        ) => throw new NotSupportedException();

        public Task<IReadOnlyList<PinChargeCategoryDto>> GetPinCategoriesAsync(
            CancellationToken ct
        ) => throw new NotSupportedException();

        public Task<IReadOnlyList<PackageTypeDto>> GetPackageTypesAsync(CancellationToken ct) =>
            throw new NotSupportedException();

        public Task<IReadOnlyList<ProviderChannelDto>> GetChannelsAsync(CancellationToken ct) =>
            throw new NotSupportedException();

        public Task<ProviderPurchaseResultDto> PurchaseAsync(
            ProviderPurchaseRequest request,
            CancellationToken ct
        ) => throw new NotSupportedException();

        public Task<ProviderTraceResultDto> TraceAsync(
            ProviderTraceRequest request,
            CancellationToken ct
        ) => throw new NotSupportedException();

        public Task<ProviderBalanceDto> GetBalanceAsync(CancellationToken ct) =>
            throw new NotSupportedException();

        public Task<IReadOnlyList<ProviderErrorDto>> GetErrorsAsync(CancellationToken ct) =>
            throw new NotSupportedException();

        public Task<ProviderReportDto> GetTransactionReportAsync(
            ProviderReportRequest request,
            CancellationToken ct
        ) => throw new NotSupportedException();
    ...
    private sealed class EmptyMarkupRuleRepository : IChargeMarkupRuleRepository
    {
        public Task<ChargeMarkupRule?> GetAsync(Guid id, CancellationToken ct = default) =>
            Task.FromResult<ChargeMarkupRule?>(null);

        public Task<IReadOnlyList<ChargeMarkupRule>> GetAllAsync(CancellationToken ct = default) =>
            Task.FromResult<IReadOnlyList<ChargeMarkupRule>>([]);

        public Task<ChargeMarkupRule?> FindApplicableAsync(
            ChargeOperator @operator,
        ...
        public Task AddAsync(ChargeMarkupRule rule, CancellationToken ct = default) =>
            Task.CompletedTask;
    ...
    private sealed class FakeChargeRequestRepository : IChargeRequestRepository
    {
        ...
        public Task<ChargeRequest?> GetAsync(Guid id, CancellationToken ct = default) =>
            Task.FromResult<ChargeRequest?>(null);
        ...
        public Task AddAsync(ChargeRequest request, CancellationToken ct = default)
        {
            AddCount++;
