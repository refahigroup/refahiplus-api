...
public sealed class CancelOrderCommandHandlerTests
{
    ...
    private static Order CreatePaidStoreOrder()
    {
        var order = Order.Create(
            ...
            Guid.NewGuid().ToString("N"),
            "StoreOrder",
            [new OrderItemData("ووچر", 50_000, 1, 0, Guid.NewGuid(), "store.voucher", null, null)]
        );
        order.MarkAsReserved(Guid.NewGuid());
        order.MarkAsPaid(Guid.NewGuid());
        order.ClearDomainEvents();
    ...
    private sealed class RefundMediator(CommandStatus status, bool blockVoucherRefund = false)
        : IMediator
    {
        ...
        )
        {
            if (request is PrepareStoreOrderRefundCommand prepare)
            {
                ...
                return Task.FromResult(
                    (TResponse)(object)new PrepareStoreOrderRefundResponse(Guid.NewGuid(), 1)
                );
            }
    ...
    private sealed class FakeOrderRepository(Order order) : IOrderRepository
    {
        ...
        public Task UpdateAsync(Order value, CancellationToken ct = default)
        {
            UpdateCount++;
            return Task.CompletedTask;
        }
