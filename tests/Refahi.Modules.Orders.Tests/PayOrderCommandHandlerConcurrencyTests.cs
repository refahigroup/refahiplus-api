...
public sealed class PayOrderCommandHandlerConcurrencyTests
{
    ...
    public async Task Concurrent_requests_with_different_request_keys_create_one_financial_intent()
    {
        ...
        var order = Order.Create(
            ...
            "create-order-key",
            "StoreInPerson",
            [new OrderItemData("خرید حضوری", 100_000, 1, 0, Guid.NewGuid(), "store.in-person", null, null)],
            financialSnapshot: new OrderFinancialSnapshotData(100_000, 0, 0, 0, 0, 100_000),
            paymentPostings:
            [
                new OrderPaymentPostingData(
                    walletId, PaymentPostingDirection.Credit, 100_000, "vendor-net")
            ]);
        ...
        var handler = new PayOrderCommandHandler(repository, mediator,
            NullLogger<PayOrderCommandHandler>.Instance, mutationLock, cancellationService);

        PayOrderCommand Command(string requestKey) => new(
            order.Id,
            userId,
            "User",
    ...
    private sealed class PaymentMediator(Guid orderId, Guid walletId, long amountMinor) : IMediator
    {
        ...
        private CommandResponse<CreatePaymentIntentResponse> CreateIntent(CreatePaymentIntentCommand command)
        {
            CreateIntentCount++;
            ReserveKey = command.IdempotencyKey;
            return new(CommandStatus.Completed, new CreatePaymentIntentResponse(
                _intentId, orderId, amountMinor, "IRR", "Reserved",
                [new AllocationResponse(walletId, amountMinor)], DateTimeOffset.UtcNow));
        }

        private CommandResponse<CapturePaymentIntentResponse> Capture(CapturePaymentIntentCommand command)
        {
            CaptureCount++;
            CaptureKey = command.IdempotencyKey;
            return new(CommandStatus.Completed, new CapturePaymentIntentResponse(
                _paymentId, _intentId, orderId, amountMinor, "IRR", "Paid",
                [new PaymentAllocationResponse(walletId, amountMinor, Guid.NewGuid())], DateTimeOffset.UtcNow));
        }
    ...
    private sealed class FakeOrderRepository(Order order) : IOrderRepository
    {
        ...
        public Task<Order?> GetByIdempotencyKeyWithItemsAsync(string idempotencyKey, CancellationToken ct = default) =>
            Task.FromResult<Order?>(null);
        public Task<List<Order>> GetByUserIdAsync(Guid userId, int page, int pageSize,
            CancellationToken ct = default) => Task.FromResult<List<Order>>([]);
        public Task<int> CountByUserIdAsync(Guid userId, CancellationToken ct = default) => Task.FromResult(0);
        public Task<List<Order>> GetAllAsync(int page, int pageSize, string? status, Guid? userId,
            string? sourceModule, IReadOnlyCollection<Guid>? allowedUserIds = null,
            CancellationToken ct = default) => Task.FromResult<List<Order>>([]);
        public Task<int> CountAllAsync(string? status, Guid? userId, string? sourceModule,
            IReadOnlyCollection<Guid>? allowedUserIds = null, CancellationToken ct = default) =>
            Task.FromResult(0);
        public Task<List<Order>> GetBySourceAsync(string sourceModule, Guid sourceReferenceId, int page,
            int pageSize, CancellationToken ct = default) => Task.FromResult<List<Order>>([]);
        public Task<int> CountBySourceAsync(string sourceModule, Guid sourceReferenceId,
